...
void load_symbols() {
    ...
    const std::string path =
        root.empty() ? std::string() : root + "/reference/sms_gmse01_funcs.txt";
    FILE* f = path.empty() ? nullptr : std::fopen(path.c_str(), "rb");
    if (f == nullptr) {
        lucent::warn("gfxdb",
                     "the US function list ({}) could not be opened, so every row in the "
...
u32 function_start(u32 addr) {
    ...
    while (lo < hi) {
        ...
        if (g_recomp_table[mid].addr <= addr)
            lo = mid + 1;
        else
            hi = mid;
    }
...
std::string symbolize(u32 addr) {
    ...
    if (start != 0 && addr != start)
        std::snprintf(off, sizeof(off), "+0x%x", addr - start);
    // The recompiler's boundary and the symbol list AGREE: a real name, and the offset is inside
    // the function it names.
    if (listed != nullptr && listed->addr == start)
        return listed->name + std::string(off);
    if (start == 0)
        return listed != nullptr ? listed->name + std::string("+0x?") : "?";
...
const char* waist_name(SbGfxWaist w) {
    switch (w) {
    case SbGfxWaist::Indexed:
        return "indexed";
    case SbGfxWaist::Immediate:
        return "immediate";
    case SbGfxWaist::J3DShape:
        return "j3d-shape";
    }
    return "?";
}
...
std::string db_path() {
    if (const char* p = std::getenv("SBR_GFXDB_PATH"))
        return p;
    const std::string root = repo_root();
    if (root.empty())
        return std::string();
...
std::string stage_label() {
    if (const char* s = std::getenv("SBR_STAGE"))
        return std::string("stage") + s;
    if (const char* s = std::getenv("SB_STAGE"))
        return std::string("stage") + s;
    if (std::getenv("SBR_FASTBOOT"))
        return "plaza";
    return "boot";
}
...
std::string join_stages(const std::string& existing, const std::string& add) {
    ...
    if (std::find(parts.begin(), parts.end(), add) == parts.end())
        parts.push_back(add);
...
void verdict(const long* row, bool auditLive, std::string& lerp, std::string& pct, long& draws) {
    ...
    const long reappearance = row[kCameraOnlyReappearance];
    if (bad == 0 && good == 0 && (birth > 0 || reappearance > 0) && row[kSnapOrtho] == 0 &&
        row[kSnapExact] == 0) {
        // Nothing was ever continuously visible and pairable. A first sighting and a return after
        // an absence are both correct to present at the current pose, but they are different facts.
        lerp = reappearance > 0 ? "discontinuous-correct" : "drew-once";
        pct = "-";
        return;
    }
    if (bad == 0 && good == 0) {
...
std::vector<Entry> read_db(const std::string& path) {
    ...
    if (!input)
        return out;
    const std::string buf{std::istreambuf_iterator<char>(input), std::istreambuf_iterator<char>()};
    for (const std::string& line : split(buf, '\n')) {
        ...
        if ((int)c.size() != kCols) {
            if (c[0] == "key")
                continue; // the header
            lucent::error("gfxdb",
                          "{} has a row with {} column(s), expected {}: '{}'. REFUSING to "
...
u8 sbr_gfxdb_site(u32 guestAddr, SbGfxWaist waist) {
    ...
    if (g_attributeTo != 0)
        guestAddr = g_attributeTo;
    if (!sbr_gfxdb_enabled() || guestAddr == 0)
    ...
    if (g_nextId >= kPopCount) {
        ...
        // known under the label that means "no seam claims this", which is the one thing the
        // audit's edge bucket must never contain.
        if (g_overflowSites.insert(guestAddr).second) {
            lucent::warn("gfxdb",
                         "population id space EXHAUSTED at {} sites; 0x{:08x} ({}) and any "
    ...
    Site site;
    site.calls = 1;
    site.name = symbolize(guestAddr);
    site.addr = guestAddr;
    site.waist = waist;
    site.used = true;
    g_sites[id] = std::move(site);
...
std::string sbr_gfxdb_symbolize(u32 guestAddr) {
    return symbolize(guestAddr);
}

void sbr_gfxdb_note_claimed(u32 guestAddr, u8 byPop) {
    if (!sbr_gfxdb_enabled() || guestAddr == 0 || byPop == SB_POP_UNLABELLED)
        return;
    if (g_attributeTo != 0)
        guestAddr = g_attributeTo;
...
void sbr_gfxdb_flush() {
    ...
    if (db.size() == 1 && db[0].key.empty())
        return; // unparseable: read_db already refused
    ...
    auto upsert = [&](const std::string& key, const std::string& kind, const std::string& symbol,
        ...
        // second identity in the file, one that only exists in runs that cannot measure it.
        // Existing rows still get their `stages` updated below; only the fabrication is refused.
        if (fresh && !auditLive) {
            ++suppressed;
            return;
        }
        ...
        if (!fresh && e.lerp != lerp && e.lerp != "unmeasured" && lerp != "unmeasured") {
            lucent::info("gfxdb",
                         "{} ({}): interpolation verdict {} -> {} ({}% of draws that "
                         "OUGHT to move do)",
                         key, symbol, e.lerp, lerp, pct);
        }
    ...
    for (int id = kFirstSiteId; id < kPopCount; ++id) {
        ...
        const char* seed =
            (fnStart != 0 && override_exists(fnStart)) ? "native-override" : "unknown";
    ...
    for (const auto& kv : g_claimed) {
        ...
        const auto at = index.find(key);
        if (at == index.end())
            continue; // never drew unclaimed, so it has no row to correct
    ...
    if (claimedRows != 0) {
        lucent::info("gfxdb",
                     "{} site row(s) re-pointed at the seam that now owns them; their "
    ...
    if (f == nullptr) {
        lucent::warn("gfxdb", "cannot write {} — the registry is not being persisted this run.",
                     tmp);
        return;
    }
    std::fprintf(f,
                 "# THE GRAPHICS REGISTRY — every graphic this port has been OBSERVED to draw.\n");
    ...
    std::fprintf(f,
                 "# lerp: yes | partial | camera-only | no | 2d-correct | discontinuous-correct | "
                 "no-primitives | unmeasured   (MEASURED, per run)\n");
    std::fprintf(f,
    ...
    for (const Entry& e : db) {
        std::fprintf(f, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n", e.key.c_str(), sanitize(e.kind).c_str(),
                     sanitize(e.symbol).c_str(), sanitize(e.stages).c_str(), sanitize(e.re).c_str(),
                     sanitize(e.lerp).c_str(), sanitize(e.first).c_str(), sanitize(e.note).c_str());
    }
    std::fclose(f);
...
void sbr_gfxdb_report() {
    ...
    for (int d = 0; d < kDispCount; ++d)
        unlabelledDraws += unlabelled[d];

    if (!sbr_gfxdb_enabled()) {
        lucent::info("gfxdb",
                     "graphics registry DISABLED (SBR_GFXDB=0): nothing was detected and "
                     ...
                     "with SBR_LERP60=1, so run the control with both set). With detection ON that "
                     "number must fall to ~0; if "
    ...
    if (unlabelledDraws != 0) {
        lucent::warn("gfxdb",
                     "{} draw(s) were still filed under (unlabelled) — no seam and no "
    ...
    if (aurora::gfx::interp::audit_disposition_count() != kDispCount) {
        lucent::error("gfxdb",
                      "aurora reports {} disposition(s), this build assumes {}. The audit "
    ...
    if (aurora::gfx::interp::max_populations() < kPopCount) {
        lucent::error("gfxdb",
                      "aurora tracks only {} population(s) but this build allocates ids up "
