...
// black-box interaction into an owned one.
//
...
struct Tally { std::atomic<unsigned long> fired{0}, drew{0}; };
...
const bool g_probe = [] {
    sb_probe_register("/screenfx", "screen-space effects: fired/drew this session, and last frame",
                      [](const ProbeArgs&) {
                          static const char* kName[5] = {"shimmer(haze)", "dash-blur", "water-refrac",
                                                         "bath-mist", "mirror-prerender"};
                          std::string out;
                          char buf[128];
                          for (int i = 0; i < 5; ++i) {
...
u32 sb_screen_effects_this_frame() { return g_published.load(std::memory_order_relaxed); }
