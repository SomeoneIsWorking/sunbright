...
// the recompiler hybrid's whole discipline is to fail FAST the instant guest state is corrupt
...
[[noreturn]] inline void sb_assert_fail(const char* expr, const char* file, int line,
                                        const char* msg) {
    std::fflush(stdout);
    std::fprintf(stderr, "\n[sunbright] ASSERT FAILED: %s\n  at %s:%d", expr, file, line);
    if (msg && *msg) std::fprintf(stderr, "\n  %s", msg);
    std::fprintf(stderr, "\n  Native backtrace (innermost first):\n");
    void* bt[96];
...
#define SB_FAIL_FMT(...) ([&]() -> const char* {                          \
        static thread_local char _sb_buf[512];                           \
        std::snprintf(_sb_buf, sizeof(_sb_buf), __VA_ARGS__);            \
        return _sb_buf; }())

#define SB_ASSERT(cond, ...)                                             \
    do { if (!(cond)) sb_assert_fail(#cond, __FILE__, __LINE__, SB_FAIL_FMT(__VA_ARGS__)); } while (0)

#define SB_FATAL(...)                                                    \
    sb_assert_fail("SB_FATAL", __FILE__, __LINE__, SB_FAIL_FMT(__VA_ARGS__))
