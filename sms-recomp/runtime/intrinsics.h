...
inline u8 sb_r8(u32 ea) {
    if (u8* p = sb_ram_fast(ea)) {
        return *p;
    }
    return mem_r8_slow(ea);
}
inline u16 sb_r16(u32 ea) {
    if (u8* p = sb_ram_fast(ea)) {
        u16 v;
        __builtin_memcpy(&v, p, 2);
        return __builtin_bswap16(v);
    }
    return mem_r16_slow(ea);
}
inline u32 sb_r32(u32 ea) {
    if (u8* p = sb_ram_fast(ea)) {
        u32 v;
        __builtin_memcpy(&v, p, 4);
        return __builtin_bswap32(v);
    }
    return mem_r32_slow(ea);
}
inline u64 sb_r64(u32 ea) {
    if (u8* p = sb_ram_fast(ea)) {
        u64 v;
        __builtin_memcpy(&v, p, 8);
        return __builtin_bswap64(v);
    }
    return mem_r64_slow(ea);
}
inline f32 sb_rf32(u32 ea) {
    u32 b = sb_r32(ea);
    f32 v;
    __builtin_memcpy(&v, &b, 4);
    return v;
}
inline f64 sb_rf64(u32 ea) {
    u64 b = sb_r64(ea);
    f64 v;
    __builtin_memcpy(&v, &b, 8);
    return v;
}
// SBR_WATCH=hex — write-watch diagnostic: any guest store touching the watched
...
// Same watchpoint, for NATIVE bulk copies into guest RAM (device DMA). A device that memcpy's into
// g_ram_base without calling this makes the watchpoint silently incomplete.
void sb_watch_range(u32 ea, u32 len, const char* who);
inline bool sb_watch_hit(u32 ea) {
    return __builtin_expect(g_watch_wa != 0, 0) && ((ea ^ g_watch_wa) & ~7u) == 0;
}
inline void sb_w8(u32 ea, u8 v) {
    if (sb_watch_hit(ea))
        sb_watch_fire(ea, v, 1, __builtin_return_address(0));
    if (u8* p = sb_ram_fast(ea)) {
        *p = v;
        return;
    }
...
inline void sb_w16(u32 ea, u16 v) {
    if (sb_watch_hit(ea))
        sb_watch_fire(ea, v, 2, __builtin_return_address(0));
    if (u8* p = sb_ram_fast(ea)) {
        u16 b = __builtin_bswap16(v);
        __builtin_memcpy(p, &b, 2);
        return;
    }
    mem_w16_slow(ea, v);
}
inline void sb_w32(u32 ea, u32 v) {
    if (sb_watch_hit(ea))
        sb_watch_fire(ea, v, 4, __builtin_return_address(0));
    if (u8* p = sb_ram_fast(ea)) {
...
inline void sb_w64(u32 ea, u64 v) {
    if (u8* p = sb_ram_fast(ea)) {
        u64 b = __builtin_bswap64(v);
        __builtin_memcpy(p, &b, 8);
        return;
    }
    mem_w64_slow(ea, v);
}
inline void sb_wf32(u32 ea, f32 v) {
    u32 b;
    __builtin_memcpy(&b, &v, 4);
    sb_w32(ea, b);
}
inline void sb_wf64(u32 ea, f64 v) {
    u64 b;
...
// Sign extend
template <int bits> inline s32 sext(u32 v) {
...
inline u32 gqr_st_type(u32 gqr) {
    return gqr & 0x7;
}
inline u32 gqr_st_scale(u32 gqr) {
    return (gqr >> 8) & 0x3F;
}
inline u32 gqr_ld_type(u32 gqr) {
    return (gqr >> 16) & 0x7;
}
inline u32 gqr_ld_scale(u32 gqr) {
    return (gqr >> 24) & 0x3F;
}
...
namespace Common {
double ApproximateReciprocal(double);
double ApproximateReciprocalSquareRoot(double);
} // namespace Common
inline f64 fres(f64 v) {
    return Common::ApproximateReciprocal(v);
}
inline f64 frsqrte(f64 v) {
    return Common::ApproximateReciprocalSquareRoot(v);
}
#else
inline f64 fres(f64 v) {
    return 1.0 / v;
...
// Resolve an address through the override-aware direct table. Direct branch sites use one
// program-wide cache per compile-time target address. Guest execution is serialized by the
// scheduler's single token, so cache initialization needs no atomic synchronization.
extern RecompFunc resolve_ppc_target(CPUState& cpu, u32 address);
template <u32 Address> inline RecompFunc g_direct_ppc_target = nullptr;
template <u32 Address> inline void call_ppc_direct(CPUState& cpu) {
    RecompFunc fn = g_direct_ppc_target<Address>;
    if (__builtin_expect(fn == nullptr, 0)) {
        fn = resolve_ppc_target(cpu, Address);
        g_direct_ppc_target<Address> = fn;
    }
    fn(cpu);
}

// Call a dynamically computed PPC address (bctrl/blrl). Direct emitted transfers use
// call_ppc_direct<Address>(); the callee returns and execution continues inline in the caller.
