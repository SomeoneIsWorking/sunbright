...
std::optional<T> parse_named(std::string_view text,
                             const std::array<NamedValue<T>, N> &values) {
...
template <typename T, std::size_t N>
std::string_view
lookup_name(T value, const std::array<NamedValue<T>, N> &values, bool display) {
  for (const auto &entry : values) {
...
bool parse_line(Settings &out, std::string_view key, std::string_view value,
                unsigned &version) {
  if (key == "version") {
    if (value == "1") {
      version = 1;
  ...
  if (input) {
    ...
    while (std::getline(input, line)) {
      ...
      if (separator == std::string::npos || separator == 0 ||
        ...
        lucent::error("settings", "{}:{}: invalid Sunbright setting '{}'",
                      path.string(), lineNumber, line);
        return false;
      }
    }
    if (!input.eof()) {
    ...
    if (version != kConfigVersion) {
      lucent::error("settings",
                    "{}: missing or unsupported config version (expected {})",
...
bool SettingsStore::save() const {
  if (m_path.empty()) {
    lucent::error("settings",
                  "cannot save settings before a path is configured");
    return false;
  }
  ...
  const auto pending = m_path.string() + ".new";
  {
    ...
    output.close();
    if (!output) {
      lucent::error("settings", "failed while writing settings file {}",
                    pending);
      return false;
    }
  }
  ...
  if (error) {
    lucent::error("settings", "cannot install settings file {}: {}",
...
void SettingsStore::apply_environment_overrides() {
  ...
  if (const char *value = std::getenv("SBR_FRAME_RATE");
