// hx_wipe_impl.cpp — native PC port of the Nintendo "Hx_" wipe/transition middleware
// (the prebuilt library that is NOT in the SMS decomp source). Replaces the link-scaffold
// stubs that used to live in native/boot_stubs/unresolved_stubs.cpp (Hx_StartWipe,
...
// markers, NOT by pixel feedback) — they are stubbed no-ops here. STAGE B will render them
// (immediate-mode GX, now captured by the renderer) to draw the "SUPER MARIO SUNSHINE" logo
// wipe on screen; the call sites and their (faithfully-computed) args are kept so that is a
// drop-in.
...
// are still computed faithfully so STAGE B can render them in place.
// ---------------------------------------------------------------------------
...
void run_callback() {
	...
	default:
		...
		// Other wipe types (e.g. type 4 Hx_Test1 at movie-end fade-out) are prebuilt
		// library callbacks not yet RE'd; they fire only after THP decode lands.
		// Honest non-completion (no faked phase advance) + a one-time warning.
