...
struct Nvk::Impl {
    ...
    // Per-texture samplers built on demand from the GC filter descriptor (mag/min/mip mode +
    // anisotropy + wrap), so a GX_LIN_MIP_LIN ground texture gets trilinear+aniso minification
    // (kills grazing-angle moire) while a GX_NEAR HUD glyph stays crisp. Keyed by sampKey().
    float devMaxAniso = 1.0f;   // device-supported max (1.0 ⇒ anisotropy off)
    std::unordered_map<uint32_t, VkSampler> samplerCache;

    uint32_t w = 0, h = 0;

    // GX filter/wrap descriptor → a cached VkSampler. magLinear: MAG is GX_LINEAR. minFilter:
    // the GX min-filter enum (encodes the mip mode). aniso: GX_ANISO_1/2/4 = 0/1/2.
    VkSampler getSampler(uint8_t magLinear, uint8_t minFilter, uint8_t aniso,
                         uint8_t wrapS, uint8_t wrapT) {
        uint32_t key = (uint32_t)(magLinear & 1)
                     | ((uint32_t)(minFilter & 7) << 1)
                     | ((uint32_t)(aniso & 3) << 4)
                     | ((uint32_t)(wrapS & 3) << 6)
                     | ((uint32_t)(wrapT & 3) << 8);
        ...
        // GX min-filter enum: 0 NEAR, 1 LINEAR, 2 NEAR_MIP_NEAR, 3 LIN_MIP_NEAR,
        //                     4 NEAR_MIP_LIN, 5 LIN_MIP_LIN.
        bool minLinear = (minFilter == 1 || minFilter == 3 || minFilter == 5);
        // Anti-aliasing enhancement: SMS ground/water textures are authored GX_LINEAR with NO
        // mip chain (mipmapCount==1), so a heavily-tiled grazing surface (the shoreline) minifies
        // many texels per pixel → textbook bilinear-without-mips moire. We generate a mip chain
        // ourselves (makeTexture) and treat GX_LINEAR min as trilinear+aniso so that surface comes
        // out smooth (matching the higher-res Dolphin-GX oracle). Point (GX_NEAR) stays crisp/no-mip
        // so pixel-art / HUD glyphs are unaffected.
        bool useMips   = (minFilter != 0);          // everything except pure GX_NEAR
        bool mipLinear = (minFilter == 1 || minFilter == 4 || minFilter == 5);
        auto toAddr = [](uint8_t wrap) -> VkSamplerAddressMode {
            switch (wrap) { case 0: return VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
                            case 2: return VK_SAMPLER_ADDRESS_MODE_MIRRORED_REPEAT;
                            default: return VK_SAMPLER_ADDRESS_MODE_REPEAT; }
        };
        VkSamplerCreateInfo s{};
        s.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
        s.magFilter = magLinear ? VK_FILTER_LINEAR : VK_FILTER_NEAREST;
        s.minFilter = minLinear ? VK_FILTER_LINEAR : VK_FILTER_NEAREST;
        s.mipmapMode = mipLinear ? VK_SAMPLER_MIPMAP_MODE_LINEAR : VK_SAMPLER_MIPMAP_MODE_NEAREST;
        s.addressModeU = toAddr(wrapS);
        s.addressModeV = toAddr(wrapT);
        ...
        if (vkCreateSampler(device, &s, nullptr, &out) != VK_SUCCESS) return VK_NULL_HANDLE;
        samplerCache.emplace(key, out);
        return out;
    }
    ...
    bool makeTexture(const uint8_t* rgba, uint32_t tw, uint32_t th,
        ...
        // Full mip chain, generated by successive 2× box-downsample blits below. Trilinear
        // minification of these levels is what removes grazing-angle moire on tiled surfaces.
        uint32_t mipLevels = 1;
        for (uint32_t m = (tw > th ? tw : th); m > 1; m >>= 1) ++mipLevels;

        VkImageCreateInfo ic{};
        ic.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
        ic.imageType = VK_IMAGE_TYPE_2D; ic.format = VK_FORMAT_R8G8B8A8_UNORM;
        ic.extent = { tw, th, 1 }; ic.mipLevels = mipLevels; ic.arrayLayers = 1;
        ic.samples = VK_SAMPLE_COUNT_1_BIT; ic.tiling = VK_IMAGE_TILING_OPTIMAL;
        // TRANSFER_SRC is needed because each level is the blit source for the next.
        ...
        auto barrier = [&](uint32_t baseLevel, uint32_t levelCount,
                           VkImageLayout from, VkImageLayout to, VkAccessFlags sa,
        ...
        // All levels UNDEFINED → TRANSFER_DST, then upload the base level.
        ...
        // Generate each successive level by a linear 2× downscale blit from the previous one.
        int32_t mw = (int32_t)tw, mh = (int32_t)th;
        for (uint32_t i = 1; i < mipLevels; ++i) {
            ...
            vkCmdBlitImage(cmd, *outImg, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                           *outImg, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &blit, VK_FILTER_LINEAR);
            // level i-1 is finished → SHADER_READ.
        ...
        barrier(mipLevels - 1, 1, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        ...
        iv.subresourceRange = { VK_IMAGE_ASPECT_COLOR_BIT, 0, mipLevels, 0, 1 };
...
bool Nvk::init(uint32_t width, uint32_t height, bool preferCpu) {
    ...
    // Enable anisotropic filtering when the device supports it (for trilinear+aniso minification
    // of grazing tiled surfaces; see Impl::getSampler). devMaxAniso stays 1.0 ⇒ off otherwise.
    VkPhysicalDeviceFeatures avail{}; vkGetPhysicalDeviceFeatures(d->phys, &avail);
    VkPhysicalDeviceFeatures want{};
    if (avail.samplerAnisotropy) {
        want.samplerAnisotropy = VK_TRUE;
        d->devMaxAniso = pp.limits.maxSamplerAnisotropy;
    }
...
bool Nvk::renderTevFrame(const std::vector<NvkTevVertex>& verts,
    ...
    std::vector<std::array<VkSampler, 8>>   batchSamp(batches.size());
    for (size_t i = 0; i < batches.size(); ++i) {
        for (int s = 0; s < 8; ++s) { batchViews[i][s] = d->tevWhiteView; batchSamp[i][s] = d->tevSampler; }
        const NvkTevBatch& b = batches[i];
        for (int s = 0; s < 8; ++s) {
            ...
            batchViews[i][s] = t.view;
            VkSampler smp = d->getSampler(b.tex[s].linear, b.tex[s].min_filter,
                                          b.tex[s].max_aniso, b.tex[s].wrap_s, b.tex[s].wrap_t);
            batchSamp[i][s] = smp ? smp : d->tevSampler;
        }
    }
    ...
    if (nsets) {
        ...
        for (uint32_t i = 0; i < nsets; ++i) {
            ...
            for (int s = 0; s < 8; ++s)
                dii[s] = { batchSamp[i][s], batchViews[i][s],
...
void Nvk::shutdown() {
    ...
    if (d->device) {
        ...
        if (d->tevSamplerLinear) vkDestroySampler(d->device, d->tevSamplerLinear, nullptr);
        for (auto& kv : d->samplerCache) vkDestroySampler(d->device, kv.second, nullptr);
        d->samplerCache.clear();
