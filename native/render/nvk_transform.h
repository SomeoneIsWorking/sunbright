...
// model pos -> Vulkan NDC xyz. mv = model-view (3x4 affine), pm = GXGetProjectionv (7),
// vp = GXGetViewportv (6). Returns NDC in ndc[3]: xy = screen px / target size * 2 - 1;
// z = GXProject's screen depth (already in [vp.nearz, vp.farz] = Vulkan [0,1] depth).
inline void nvk_project_ndc(const float pos[3], float mv[3][4],
                            const float* pm, const float* vp, float ndc[3]) {
    float sx, sy, sz;
...
inline std::vector<NvkVertex> nvk_project_mesh(
    ...
    for (unsigned i : indices) {
        ...
        out.push_back({ ndc[0], ndc[1], ndc[2],
