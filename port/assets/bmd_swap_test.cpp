...
int main() {
	...
	// Build a synthetic big-endian BMD: 0x20 header + one 0x28-byte INF1 block
	// (header 0x08 + fields 0x10 + a 3-entry hierarchy = 0x18..0x24).
	const uint32_t INF1_SIZE = 0x28;
	const uint32_t TOTAL = 0x20 + INF1_SIZE;
	std::vector<uint8_t> be(TOTAL, 0);
	put32(be, 0x00, 0x4A334432);          // 'J3D2'
	put32(be, 0x04, 0x626D6433);          // 'bmd3'
	put32(be, 0x08, TOTAL);               // fileSize
	put32(be, 0x0C, 1);                   // blockNum = 1
	// INF1 block at 0x20
	put32(be, 0x20, 0x494E4631);          // 'INF1'
	put32(be, 0x24, INF1_SIZE);           // block size
	put16(be, 0x20+0x08, 0x0007);         // mFlags
	put32(be, 0x20+0x0C, 0x00000003);     // mPacketNum
	put32(be, 0x20+0x10, 0x00000011);     // mVtxNum = 17
	put32(be, 0x20+0x14, 0x00000018);     // mpHierarchy offset (block-relative)
	// hierarchy entries at block+0x18: {type,value} pairs, terminated by type=0
	put16(be, 0x20+0x18, 0x0010); put16(be, 0x20+0x1A, 0x0000);  // joint #0
	put16(be, 0x20+0x1C, 0x0011); put16(be, 0x20+0x1E, 0x0001);  // material #1
	put16(be, 0x20+0x20, 0x0000); put16(be, 0x20+0x22, 0x0000);  // terminator

	std::vector<uint8_t> out;
	BmdSwapResult r = bmd_swap_to_host(be.data(), be.size(), out);

	CK(r.ok, "swap ok");
	CK(r.block_num == 1, "block_num == 1");
	CK(r.blocks_covered == 1 && r.all_covered, "INF1 covered + all_covered");
	...
	CK(h32(out.data()+0x0C)==1,          "blockNum host-readable");
	CK(h32(out.data()+0x20)==0x494E4631, "INF1 tag host-readable");

	// INF1 fields.
	CK(h16(out.data()+0x20+0x08)==0x0007, "mFlags");
	CK(h32(out.data()+0x20+0x0C)==3,      "mPacketNum");
	CK(h32(out.data()+0x20+0x10)==0x11,   "mVtxNum");
	CK(h32(out.data()+0x20+0x14)==0x18,   "mpHierarchy");

	// Hierarchy entries.
	CK(h16(out.data()+0x20+0x18)==0x0010, "hier[0].type");
	CK(h16(out.data()+0x20+0x1C)==0x0011, "hier[1].type");
	CK(h16(out.data()+0x20+0x1E)==0x0001, "hier[1].value");
