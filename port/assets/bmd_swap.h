...
// VTX1 + SHP1 implemented & verified (synthetic bmd_swap_test AND real BMDs via
...
// and s16 positions); SHP1 swaps all structural fields (init data, vtx-desc
// lists, mtx/draw init data) but DEFERS the display-list byte-stream interior
// (BE u16 counts/indices) to the render path (needs VCD-driven parse; GX not
// owned yet). The remaining blocks (MAT3/TEX1) are stubbed — see bmd_swap.cpp
// for the per-block field-map plan.
