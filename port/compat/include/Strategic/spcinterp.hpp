// SHADOW of reference/sms/include/Strategic/spcinterp.hpp — keep in sync.
//
// Reason: TSpcSlice declares an accessor `u32 typeof() const`. `typeof` is a
// reserved GNU keyword (the typeof OPERATOR) under -std=gnu++17, so this
// member-function declaration is misparsed ("expected primary-expression
// before ')'"). CodeWarrior accepted `typeof` as a plain identifier.
//
// Fix: rename the accessor to `getType()` (semantically identical). Only the
// declaration site is changed; this header is shadowed earlier on the -I path
// so it wins resolution while the pristine decomp header stays untouched.
// NOTE: the .cpp callers (spcinterp.cpp, EventWatcher.cpp) still spell the call
// `.typeof()` and are NOT in the core build; if they are ever added, their
// owned copies must be updated to `.getType()` (and would otherwise also hit
// the keyword error on the call site under gnu++17).
