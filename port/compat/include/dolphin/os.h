...
#if !DEBUG
/* SHADOW (64-bit port): widen the internal arithmetic to uintptr_t so a
 * pointer-typed `paddr` is not truncated to 32 bits before forming the cached
 * virtual address. Result is still a void*; behavior is unchanged on a 64-bit
 * little-endian target for GC-range addresses. */
