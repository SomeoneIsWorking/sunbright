// SHADOW of reference/sms/include/JSystem/JSupport/JSUList.hpp.
// Identical to the decomp EXCEPT the four JSUTree<T> child-mutator wrappers
// (appendChild/prependChild/removeChild/insertChild) call append/prepend/
// remove/insert UNQUALIFIED. Those names live in the dependent base JSUList<T>
// (JSUTree<T> : public JSUList<T>, public JSULink<T>). CodeWarrior resolved
// them laxly; g++ two-phase name lookup does NOT look into dependent bases for
// unqualified names ("'append' was not declared in this scope" at instantiation
// -[fpermissive]). Fix: qualify with this-> so the lookup is deferred to the
// dependent base. Pure name-lookup fix, no behavior change. Keep in sync with
// the decomp.
#ifndef JSU_LIST_H
#define JSU_LIST_H
