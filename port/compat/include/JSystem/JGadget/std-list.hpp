...
// PORT SHADOW of reference/sms/include/JSystem/JGadget/std-list.hpp.
// Faithful copy + ONE minimal fix: in the nested class
// TList_pointer<T>::iterator, the inner `typedef Base::iterator Base;`
// re-declared the name `Base` (already meaning the outer
// `typedef TList_pointer_void Base;`, used in the base-specifier
// `class iterator : Base::iterator`) to a *different* type within the same
// scope. CodeWarrior tolerated this; g++ rejects it with
// "declaration of 'Base' changes meaning of 'Base'" [-Wchanges-meaning].
// Fix: rename the inner typedef to `BaseIt` and update the in-class uses
// (base-specifier, ctor param/init, the Base:: member calls). No semantic
// change — `BaseIt` resolves to exactly the same type
// (TList_pointer_void::iterator) the old inner `Base` did.
...
public:
	class iterator : Base::iterator {
		...
		typedef Base::iterator BaseIt; // PORT: was `typedef Base::iterator Base;`
	...
	public:
		...
		T operator*() { return (T)this->BaseIt::operator*(); }
		T operator->() { return *(T*)this->BaseIt::operator->(); }

		iterator& operator++()
		{
			this->BaseIt::operator++();
			return *this;
		}
		iterator operator++(int) { return iterator(this->BaseIt::operator++(0)); }
