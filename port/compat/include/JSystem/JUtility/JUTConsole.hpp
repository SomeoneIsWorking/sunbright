// SHADOW of reference/sms/include/JSystem/JUtility/JUTConsole.hpp.
// Identical to the decomp EXCEPT the non-MWERKS ConsoleList typedef (below)
// uses `-sizeof(JKRDisposer)` as the TLinkList<T,int I> offset argument.
// sizeof yields size_t (unsigned long, 8 bytes on LP64); negating it produces a
// huge unsigned value (18446744073709551568) that narrows to the `int` template
// parameter -> -Wnarrowing error (CodeWarrior, where this branch is never taken
// since __MWERKS__ uses offsetof, never saw it). Fix: cast the size to (int)
// before negating, so `-(int)sizeof(JKRDisposer)` is the intended small negative
// offset (= -offsetof(JUTConsole, mLinkNode), since mLinkNode follows the
// JKRDisposer base subobject). Behavior-preserving. Keep in sync with the decomp.
