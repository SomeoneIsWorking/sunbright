...
// reference/sms/include/JSystem/JAudio/JAInterface/JAIBasic.hpp — keep in sync.
// 64-bit-port change ONLY: checkDvdLoadArc's arg is declared `unsigned long`
// here but defined `u32` in JAIGFrameSequence.cpp, and it is passed as the
// `void(*)(u32)` callback to Vload::loadFileAsync (JAIBasic.cpp:1182). The arg
// is a packed load ID (not a pointer): caller builds it as `param_2 | 0xFE |
// uVar1<<16 | unaff_r28<<8`. On GC long==u32==4 bytes so it matched; on LP64
// `unsigned long`==8 bytes breaks both the def-match and the callback-type. The
// declaration is corrected to `u32` to match the definition. (checkPlayingSeqTrack's
// `unsigned long` is left as-is — its definition also spells `unsigned long`, so
// it still matches and is not on the failing callback path.) Verbatim otherwise.
#ifndef JAIBASIC_HPP
#define JAIBASIC_HPP
