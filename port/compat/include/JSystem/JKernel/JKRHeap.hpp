...
// reference/sms/include/JSystem/JKernel/JKRHeap.hpp — keep in sync.
// ONLY 64-bit-port change: dispose_subroutine(begin, end) receives a HOST
// address RANGE (its body compares `(void*)begin <= disposer < (void*)end`
// against live JKRDisposer pointers), so its params are widened from u32 to
// uintptr_t to avoid truncating the host addresses on LP64. It is a private
// method called only from JKRHeap.cpp's dispose() overloads, so no other TU is
// affected. operator new/delete already use size_t. Verbatim otherwise.
#ifndef JKR_HEAP_H
#define JKR_HEAP_H
