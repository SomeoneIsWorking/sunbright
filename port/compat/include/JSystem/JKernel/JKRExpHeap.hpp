...
// reference/sms/include/JSystem/JKernel/JKRExpHeap.hpp — keep in sync.
// ONLY 64-bit-port change: CMemBlock::getBlock() did pointer arithmetic via a
// `(u32)data` intermediate, truncating the host pointer on LP64. Widened the
// intermediate to `uintptr_t` (pure address arithmetic, result is still a
// CMemBlock*; behavior unchanged on any target). Verbatim otherwise.
