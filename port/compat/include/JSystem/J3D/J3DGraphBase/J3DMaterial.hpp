...
public:
	...
	{
		// SHADOW (64-bit port): the original GC code gated on
		// `(u32)unk38 < 0xC0000000` — a GameCube address-space test that
		// accepts cached-RAM pointers (0x8000_0000..0xBFFF_FFFF) and rejects
		// uncached-mirror/garbage addresses. In this codebase unk38 is only
		// ever assigned nullptr or a real J3DMaterialAnm* (setMaterialAnm /
		// J3DModel / loaders), so for every value that actually occurs the gate
		// is a no-op: 0 < 0xC0000000 returns nullptr, a valid cached pointer
		// returns itself. On a 64-bit host real pointers exceed 0xC0000000, so
		// keeping the literal threshold would wrongly reject every anm. The
		// behavior-preserving translation is therefore to return the pointer as
		// stored (no truncation). The GC range test had no effect on the inputs
		// this engine produces and cannot be expressed as a host address range.
		return unk38;
	}
