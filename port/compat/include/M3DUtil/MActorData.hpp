...
// PORT SHADOW of reference/sms/include/M3DUtil/MActorData.hpp.
// Faithful copy + ONE minimal fix in the template body
// MActorAnmDataEach<T>::loadAnmPtrArray: `sortByFileNameRaw(unkC)` passes
// `unkC` (J3DAnmBase**) to a parameter typed `void**`. C++ does NOT implicitly
// convert J3DAnmBase** -> void** (only single-level T* -> void*), so g++ rejects
// it: "cannot convert 'J3DAnmBase**' to 'void**'" [-Wtemplate-body]. CodeWarrior
// accepted the conversion. sortByFileNameRaw is type-erased — it only reorders
// the pointer array (never dereferences as J3DAnmBase); J3DAnmBase** and void**
// are layout-identical, so the explicit `(void**)` cast is behavior-preserving.
// (The snprintf-undeclared error in this same template body is fixed separately
//  by the port/compat/include/printf.h shadow.)
...
public:
	...
	void loadAnmPtrArray(const char* param_1, const char* param_2)
	{
		...
		char trash[8];
		sortByFileNameRaw((void**)unkC); // PORT: explicit cast (was implicit under CodeWarrior)
	}
