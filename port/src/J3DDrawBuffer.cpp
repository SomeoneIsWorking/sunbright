// Owned portable copy of reference/sms/src/JSystem/J3D/J3DGraphBase/J3DDrawBuffer.cpp.
// Identical to the decomp EXCEPT the two 64-bit pointer-truncation sites where a
// pointer is used as a HASH key into the mat-packet bucket array:
//   entryMatSort:     hash = (u32)texture->getResTIMG(texNo)
//   entryMatAnmSort:  slot = (u32)pMaterialAnm & (mSize - 1)
// Casting the pointer to u32 is lossy on LP64 (x86-64/arm64) and a hard error
// without -fpermissive. Routed through uintptr_t (full pointer width) then
// truncated to u32 — the value is purely a hash/slot key (masked into the
// bucket array by `& (mSize - 1)`), so this is behavior-preserving on the GC
// and now correct on 64-bit. The `& (mSize - 1)` masking intent is preserved.
