// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASDvdThread.cpp —
// keep in sync. 64-bit-port changes: (1) the ARQ DMA source (host RAM buffer
// `buf`) and dest (`call->unk24`) passed to ARQPostRequest were cast through
// (u32); widened to uintptr_t to match the dolphin/ar.h shadow. (2) the
// aramDmaFinish/aramDmaFinish2 ARQ completion callbacks (one is passed as the
// ARQCallback, which is now void(*)(uintptr_t)) take their arg as uintptr_t to
// match the widened typedef — both ignore the arg, so behavior is unchanged.
// Verbatim otherwise.
#include <stdint.h>
...
static void Dvd::aramDmaFinish2(uintptr_t param) { }
