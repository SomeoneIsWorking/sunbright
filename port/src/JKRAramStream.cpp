// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRAramStream.cpp — keep in
// sync. 64-bit-port change ONLY: the transfer `buffer` is a HOST main-RAM
// pointer; the two `(u32)buffer` casts (the JKRAramPcs source arg, now
// uintptr_t per the JKRAramPiece.hpp shadow; and the ALIGN_NEXT of the
// trans-buffer base) are widened to uintptr_t so the pointer is not truncated.
// The public addr/size/offset params are ARAM-space and stay u32. Verbatim
// otherwise.
