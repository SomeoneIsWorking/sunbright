// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRDvdRipper.cpp — keep in sync.
// ONLY 64-bit-port change: two `(u8*)ALIGN_NEXT((u32)buffer, 32)` sites align a
// stack buffer through a (u32) cast, truncating the host pointer on LP64.
// Widened the intermediate to uintptr_t (result casts back to u8*). Verbatim
// otherwise.
#include <stdint.h>
