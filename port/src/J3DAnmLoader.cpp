// Owned portable copy of reference/sms/src/JSystem/J3D/J3DGraphLoader/J3DAnmLoader.cpp.
// Identical to the decomp EXCEPT four 64-bit pointer-truncation sites (two
// identical loop bodies, in the v-color-index relocation of the color and the
// full-color anim loaders) of the form:
//   mpData = (void*)((s32)indexPtr0 + (s32)mpData * 2)
// where indexPtr0/indexPtr1 are real runtime BASE pointers (from
// JSUConvertOffsetToPtr) and mpData holds a small file offset. Casting the base
// pointer to s32 and back to void* truncates it on LP64 (x86-64/arm64) — a hard
// error without -fpermissive AND a genuine 64-bit-correctness bug (high 32 bits
// of the base lost). Both the base AND the mpData offset (itself a void*-typed
// small offset) are routed through uintptr_t so the base keeps full width; the
// `* 2` offset scaling is preserved verbatim. Keep in sync with the decomp.
#include <cstdint>
