// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASTrack.cpp — keep in
// sync. 64-bit-port changes:
//  (1) TTrack::mainProc has `goto bail;` that, on the native (non-CodeWarrior)
//      compiler, jumps INTO the scope of `u32 r31 = 0;` past its initializer —
//      ill-formed in C++ (CodeWarrior was lax). Hoisted r31's declaration above
//      the goto and initialized it to 0 there (the goto path always wants r31==0,
//      which is what the in-scope init produced), and removed the inner
//      re-declaration. Behavior-preserving.
//  (2) OSf32tos8 (float -> saturated s8 via a Gekko paired-single quantized
//      store, GQR type 6 = s8) is declared in dolphin/os.h ONLY under
//      `#ifdef GEKKO` with a `#ifdef __MWERKS__` (asm) body — so it is absent on
//      the native build, and os.h may not be edited. Provided here as a faithful
//      file-local port (round-to-nearest, clamp to [-128,127]). PAL ITEM: hoist
//      this to port/pal/os/ (alongside the other GQR/fast-cast math) so every TU
//      using OSf32tos8 shares one definition.
// Verbatim otherwise.
#include <cmath>
...
#ifndef GEKKO
// 64-bit port: faithful native OSf32tos8 (see file header note (2)). The Gekko
// `psq_st f1, 0(out), 1, GQR4` with GQR4 = type 6 / scale 0 = a single signed
// 8-bit quantized store: round-to-nearest then saturate to [-128, 127].
// PAL ITEM: move to port/pal/os/.
static inline void OSf32tos8(f32* f, s8* out)
{
	f32 v = *f;
	long r = (long)(v < 0.0f ? v - 0.5f : v + 0.5f); // round to nearest
	if (r > 127)
		r = 127;
	else if (r < -128)
		r = -128;
...
s8 TTrack::mainProc()
{
	...
	// 64-bit port: hoisted above the `goto bail` so the jump does not cross an
	// initializer (ill-formed in C++). The goto path leaves r31 == 0, matching
	// the original in-scope `u32 r31 = 0;`.
	u32 r31 = 0;
