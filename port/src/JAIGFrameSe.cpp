// OWNED COPY of reference/sms/src/JSystem/JAudio/JAInterface/JAIGFrameSe.cpp —
// keep in sync. 64-bit-port change ONLY: JAISound::unk24 is a `const Vec*` but
// the GC SDK MTXMultVec(Mtx44, Vec* src, Vec* dst) takes a non-const src;
// CodeWarrior accepted the const arg, g++ does not. MTXMultVec only READS src, so
// an explicit `(Vec*)` cast is behavior-preserving (same pattern as the README's
// MActorData.hpp `(void**)` cast). Verbatim otherwise.
