// Owned portable copy of reference/sms/src/JSystem/JRenderer.cpp.
// Identical to the decomp EXCEPT the two 64-bit pointer-truncation sites in
// J3DGDSetTexImgPtr / J3DGDLoadTlut: `(u32)OSPhysicalToCached(ptr) >> 5` casts
// a void* to u32 before the >>5, which is lossy on LP64 (x86-64/arm64) and a
// hard error without -fpermissive. Routed through uintptr_t so the widening
// happens BEFORE the shift; the final value is then truncated to u32 to fill
// the BP-command register field (the decomp's own intent — the GPU register is
// 32-bit, and on the GC the physical address is <512MB so >>5 already fits 27
// bits). Keep in sync with the decomp.
#include <cstdint>
