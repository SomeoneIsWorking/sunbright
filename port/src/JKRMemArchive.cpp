// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRMemArchive.cpp — keep in
// sync. 64-bit-port fixes (pointer-truncation class):
//   * mArchiveData = (u8*)((u32)mArcHeader + ...): REAL pointer arithmetic that
//     would truncate the header pointer on LP64 -> widened to (uintptr_t). Bug
//     fix, not just well-formedness.
//   * JKRArchive((s32)buffer,...) / check_mount_already((s32)param_1) /
//     fixedInit((s32)param_1): a pointer stashed into the s32 mEntryNum field.
//     FLAG: lossy-by-design (a 64-bit pointer cannot fit in s32). It is VESTIGIAL
//     in the mem path — the mem ctor opens via open(void*,...) (the buffer
//     overload), never open(s32 entryNum), so mEntryNum is never dereferenced as
//     a pointer here. Widened through (uintptr_t) to be well-formed; matches the
//     decomp's "// WTF?" intent. A real fix would widen mEntryNum, but that field
//     is shared with the DVD entry-number path (PAL/struct-width work, not here).
...
JKRMemArchive::JKRMemArchive(void* buffer, u32 bufferSize,
    ...
    : JKRArchive((s32)(uintptr_t)buffer, MOUNT_MEM) // WTF? (vestigial; see header note)
{
...
{
	if (check_mount_already((s32)(uintptr_t)param_1)) {
		return false;
	}
	fixedInit((s32)(uintptr_t)param_1);
