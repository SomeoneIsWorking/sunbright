// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASChAllocQueue.cpp —
// keep in sync. 64-bit-port change ONLY: TDSPChannel::alloc's second param is a
// HOST pointer (widened to uintptr_t in the JASDSPChannel.hpp shadow), so the
// `(u32)chan` cast at the deQueue alloc call is widened to `(uintptr_t)chan` to
// avoid truncating the TChannel* on LP64. Verbatim otherwise.
#include <stdint.h>
