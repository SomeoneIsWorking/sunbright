// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASDSPInterface.cpp —
// keep in sync. ONLY 64-bit-port change: setupBuffer passes the channel buffer,
// the two filter tables, and the FX buffer (all HOST pointers) to DsetupTable
// via (u32) casts — widened to uintptr_t to match the dspproc.h shadow (and the
// PAL DsetupTable definition). Verbatim otherwise.
#include <stdint.h>
...
namespace DSPInterface {
	...
	void setupBuffer()
	{
		DsetupTable(64, (uintptr_t)CH_BUF, (uintptr_t)&DSPRES_FILTER,
		            (uintptr_t)&DSPADPCM_FILTER, (uintptr_t)FX_BUF);
	}
	void initBuffer()
	{
