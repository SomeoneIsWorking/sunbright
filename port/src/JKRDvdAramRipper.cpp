// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRDvdAramRipper.cpp — keep in
// sync. 64-bit-port changes: (1) loadToAram_Async's callback param + the
// `mCallback((u32)command)` invocation carry a host JKRADCommand* — widened to
// uintptr_t to match the JKRDvdAramRipper.hpp shadow. (2) `(u32)&buffer` (stack
// buffer alignment) and `(u32)dmaBuf` (host DMA source passed to JKRAramPiece::
// orderSync, whose source param is now uintptr_t) widened to uintptr_t.
// Verbatim otherwise.
#include <stdint.h>
