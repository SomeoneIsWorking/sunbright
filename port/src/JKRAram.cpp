// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRAram.cpp — keep in sync.
// 64-bit-port changes: every host main-RAM pointer fed to JKRAramPcs (the ARAM
// DMA bridge) was cast through (u32); on LP64 that truncates the pointer. The
// JKRAramPiece.hpp shadow widened the source/destination params to uintptr_t,
// so each host-pointer cast here is widened to uintptr_t to match: source ptrs
// (allocatedMem/buf), destination ptrs (bufPtr/szpSpace/rv/buffer/dest), the
// stack-buffer alignment, and the IS_ALIGNED check on `addr`. ARAM addresses
// (`address`, `srcAddress`) stay u32 — genuine 32-bit hardware addresses.
// Verbatim otherwise.
#include <stdint.h>
...
static u8* nextSrcData(u8* current)
{
	...
	JKRAramPcs(1, (u32)(srcAddress + srcOffset), ((uintptr_t)dest + left),
