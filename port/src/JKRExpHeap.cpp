// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRExpHeap.cpp — keep in sync.
// 64-bit-port changes: (1) all heap-block address arithmetic that cast a
// CMemBlock*/void* through (u32) — offset/size computations, the block-walk
// bound comparison, joinTwoBlocks' endAddr/nextAddr, and allocFromTail's `start`
// (held an address used as `(CMemBlock*)start - 1`, so the local was widened to
// uintptr_t too) — widened to uintptr_t so host pointers are not truncated on
// LP64. (2) the two `checkCode += (u32)block * 3` are a STATE CHECKSUM, kept u32
// but the pointer input is widened via uintptr_t first. (3) CMemBlock::allocBack
// was DEFINED with `unsigned long size` while the header declares `u32 size`
// (long==4 on GC, 8 on LP64 → the definition no longer matched the
// declaration); ported the definition's param to u32. Verbatim otherwise.
#include <stdint.h>
...
void* JKRExpHeap::allocFromTail(u32 size, int align)
{
	...
	u32 usedSize;
	// 64-bit port: `start` is a HEAP ADDRESS (used as `(CMemBlock*)start - 1`),
	// not a size — widened to uintptr_t so the host pointer is not truncated.
	uintptr_t start;
