// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRHeap.cpp — keep in sync.
// 64-bit-port changes:
//  (1) operator new / operator new[] were DEFINED taking `u32 byteCount`, but a
//      global operator new must take `size_t` (size_t==u32 on GC, but 8 bytes on
//      LP64) — and the JKRHeap.hpp declarations already say size_t. Ported the
//      definitions' param to size_t. (forwarded to JKRHeap::alloc, which takes a
//      u32 size — the arena is < 4 GB, so the narrowing to the alloc arg is fine.)
//  (2) dispose_subroutine / dispose: begin/end are HOST address ranges (compared
//      against live disposer pointers), so they + the (u32)pointer casts feeding
//      them are widened to uintptr_t (matches the JKRHeap.hpp shadow).
//  (3) initArena: OSRoundUp32B/OSRoundDown32B are os.h MACROS that cast their
//      arg through (u32) (truncating the host arena pointer); since the os.h
//      shadow may not be edited, they are #undef'd and redefined here with
//      uintptr_t arithmetic before use. `*outUserRamSize` stays u32 (an arena
//      size) but its pointer subtraction is widened via uintptr_t.
// Verbatim otherwise.
#include <stdint.h>
...
// (3) — re-point the 32-byte round macros to pointer-width arithmetic. The os.h
// shadow is off-limits; the GC versions truncate a host pointer through (u32).
#undef OSRoundUp32B
#undef OSRoundDown32B
