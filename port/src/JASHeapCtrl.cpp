// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASHeapCtrl.cpp — keep
// in sync. 64-bit-port changes ONLY: (1) THeap::initMotherHeap's 1st param is the
// HEAP BASE host pointer (stored into u8* unk8), widened u32 -> uintptr_t to match
// the JASHeapCtrl.hpp shadow; (2) TSolidHeap::init aligns the `mem_start` HOST
// pointer, so the two `(u32)mem_start` casts become `(uintptr_t)mem_start` to
// avoid truncation on LP64. Verbatim otherwise.
...
namespace Kernel {
	...
	void THeap::initMotherHeap(uintptr_t param_1, u32 param_2, u8 param_3)
	{
