// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRDvdFile.cpp — keep in sync.
// ONLY 64-bit-port change: JKRDvdFile::sync()'s `return (s32)m;` where `m` is an
// OSMessage (void*) carrying an s32 result (posted as `(OSMessage)result` in
// doneProcess). `(s32)void*` is an ill-formed narrowing cast on LP64; widened to
// `(s32)(intptr_t)m`. Verbatim otherwise.
