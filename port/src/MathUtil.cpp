// Owned portable copy of reference/sms/src/MarioUtil/MathUtil.cpp —
// inline PPC asm reimplemented in portable C++. Keep in sync with the decomp.
//
// The two paired-single inline-asm functions at the end of the decomp file
// (MsVECMag2, MsVECNormalize) are reimplemented below in portable C++ with the
// EXACT Gekko numeric semantics (raw frsqrte estimate, NO Newton refinement,
// fsel zero-guard) — see the per-function notes. Everything above is a verbatim
// copy of the decomp body (no asm there).
//
// NOTE for the build integrator: port/pal/mtx.cpp ALSO defines MsVECMag2 /
// MsVECNormalize (it stood in for this deferred TU while it had asm). When this
// owned MathUtil.cpp enters the core build, the duplicate definitions in
// pal/mtx.cpp MUST be dropped (or this file kept out of smsport_pal's link) to
// avoid a multiple-definition link error. The implementations are identical.
...
// ---------------------------------------------------------------------------
// Portable reimplementations of the two paired-single inline-asm functions.
//
// MsVECMag2 — vector magnitude using the RAW Gekko frsqrte estimate (NO
// Newton-Raphson refinement; this is the fast/approx path MarioUtil uses).
//
//   Original asm:
//     sq = x*x + y*y + z*z            (ps_mul / ps_madd / ps_sum0)
//     r  = frsqrte(sq)                (raw ~12-bit estimate)
//     r  = fsel(-sq, sq, r)           (sq==0 -> 0 ; sq>0 -> r)
//     return sq * r                   (== sq/sqrt(sq) == sqrt(sq))
//
// The fsel guards the sq==0 case: fsel(a,b,c) = (a>=0)?b:c, with a=-sq, so
// sq==0 yields sq (0), sq>0 yields the estimate. __frsqrte is the bit-accurate
// Gekko estimate from compat/intrinsics.h (not 1/sqrt) so the result matches
// the recompiled game.
f32 MsVECMag2(register Vec* v)
{
	f32 sq    = v->x * v->x + v->y * v->y + v->z * v->z;
	f32 rsqrt = (f32)__frsqrte((f64)sq);
	f32 sel   = (-sq >= 0.0f) ? sq : rsqrt; // fsel(-sq, sq, rsqrt)
	return sq * sel;
}

// MsVECNormalize — normalize v1 into v2 using the RAW frsqrte estimate (again
// NO Newton refinement; matches the asm exactly).
//
//   Original asm:
//     sq = x*x + y*y + z*z            (ps_mul / ps_madd / ps_sum0)
//     r  = frsqrte(sq)                (raw estimate)
//     v2.xy = v1.xy * r               (ps_muls0)
//     v2.z  = v1.z  * r               (fmuls)
void MsVECNormalize(register Vec* v1, register Vec* v2)
{
	f32 sq    = v1->x * v1->x + v1->y * v1->y + v1->z * v1->z;
	f32 rsqrt = (f32)__frsqrte((f64)sq); // raw estimate, no refine
	v2->x = v1->x * rsqrt;
	v2->y = v1->y * rsqrt;
	v2->z = v1->z * rsqrt;
}
