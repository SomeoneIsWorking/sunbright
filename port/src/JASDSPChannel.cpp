// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASDSPChannel.cpp —
// keep in sync. 64-bit-port change: TDSPChannel::unk8 holds a host TChannel*
// (see the JASDSPChannel.hpp shadow), so the params that carry it — allocate's
// `param`, alloc's `param2`, free's `param` — are widened from u32 to uintptr_t
// to match the widened declarations / avoid truncating the pointer. Bodies are
// verbatim (the assignments/comparisons are width-agnostic). alloc's `param1`
// (a flag) stays u32. Verbatim otherwise.
