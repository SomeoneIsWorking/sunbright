// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASAiCtrl.cpp — keep
// in sync. 64-bit-port change ONLY: the AI DMA start address is a HOST main-RAM
// pointer (the s16* DAC ring buffers `dac[2]` / `useRspMadep`), so the two
// `(u32)`/`u32(...)` casts feeding AIInitDMA are widened to `(uintptr_t)` to
// match the widened AIInitDMA signature in the dolphin/ai.h shadow (avoids
// truncating the host pointer on LP64). Verbatim otherwise.
#include <stdint.h>
