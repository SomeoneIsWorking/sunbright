// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRDvdArchive.cpp — keep in
// sync. ONLY 64-bit-port change (pointer-truncation class): the three
// `(int)&mArcInfoBlock->num_nodes + offset` casts compute a derived pointer from
// a REAL pointer; `(int)ptr` truncates it on LP64 -> widened to (uintptr_t)
// (bug fix). Verbatim otherwise.
...
bool JKRDvdArchive::open(s32 entryNum)
{
	...
	mDirectories = (SDIDirEntry*)((uintptr_t)&mArcInfoBlock->num_nodes
	                              + mArcInfoBlock->node_offset);
	mFileEntries = (SDIFileEntry*)((uintptr_t)&mArcInfoBlock->num_nodes
	                               + mArcInfoBlock->file_entry_offset);
	mStrTable    = (char*)((uintptr_t)&mArcInfoBlock->num_nodes
