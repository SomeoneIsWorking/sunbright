// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASCalc.cpp — keep in
// sync. ONLY 64-bit-port change: bcopy/bzero test buffer alignment via
// `reinterpret_cast<uintptr_t>(ptr) & mask`, which truncates the host pointer on LP64.
// Widened the cast to uintptr_t (only the low alignment bits are used; behavior
// unchanged). Verbatim otherwise.
#include <stdint.h>
