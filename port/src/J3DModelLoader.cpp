// Owned portable copy of reference/sms/src/JSystem/J3D/J3DGraphLoader/J3DModelLoader.cpp.
// Identical to the decomp EXCEPT the 64-bit pointer-truncation sites, all
// `(u32)ptr`-style casts that are lossy on LP64 (x86-64/arm64) and hard errors
// without -fpermissive. Routed through uintptr_t, by category:
//   * Offset-into-block fields stored in void*-typed members (mpIndexTable,
//     mpShapeInitData): the value is a small file offset, not a runtime
//     pointer — widen to uintptr_t to make the cast legal (semantically an
//     offset; fits u32 either way).
//   * Pointer DIFFERENCE element counts ((u32)nrm_end - (u32)mVtxNormArray)/12,
//     and (u32)color0_end - (u32)mVtxColorArray[0])/4): both operands are real
//     runtime pointers. Subtract as uintptr_t (widened FIRST) so the high bits
//     are not lost before the subtraction — the genuine 64-bit-correctness fix.
//   * Block-size minus offset ((i_block->mSize - (u32)i_block->mpVtxNrmArray)):
//     mSize is a u32 byte size, the field holds a small file offset — widen the
//     offset to uintptr_t for a legal cast.
//   * Pointer >>4 stored in J3DMaterial::unk18 (a matrix-cache index encoding):
//     widen to uintptr_t before the shift, then truncate to the u32 field.
//   * Pointer + index stored in unk18 ((u32)mMaterials + materialID): widen the
//     base to uintptr_t; the sum is truncated into the u32 field (the decomp's
//     own data model — unk18 is 4 bytes).
// All masking/shift intent is preserved verbatim. Keep in sync with the decomp.
#include <cstdint>
