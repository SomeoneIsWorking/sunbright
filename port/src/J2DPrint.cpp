// Owned portable copy of reference/sms/src/JSystem/J2D/J2DPrint.cpp.
// Identical to the decomp EXCEPT four 64-bit pointer-truncation sites, all of
// the same shape: a byte-distance between two pointers computed by casting each
// operand to u32 BEFORE subtracting (`(u32)pa - (u32)pb`). On the 32-bit GC the
// pre-subtract truncation was a no-op; on LP64 (x86-64/arm64) it lops off the
// high 32 bits of each pointer, and the cast is a hard error without
// -fpermissive. Fix: widen each operand to uintptr_t before subtracting so the
// difference is computed at full pointer width (the difference itself is small
// and the comparison semantics are preserved). Sites:
//   J2DPrint::parse     line 278/326: ((uintptr_t)param_1 - (uintptr_t)local_c)
//   J2DPrint::getNumber line 604/605: ((uintptr_t)local_28 - (uintptr_t)*param_1)
// Keep in sync with the decomp.
#include <cstdint>
