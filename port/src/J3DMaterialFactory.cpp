// OWNED COPY of reference/sms/src/JSystem/J3D/J3DGraphLoader/J3DMaterialFactory.cpp
// — keep in sync. ONLY 64-bit-port change: line ~17, the offset-distance check
// `((u32)block.mpIndInitData - (u32)block.mpNameTable) > 4` casts two pointer-
// typed (but file-offset-holding) fields to u32; `(u32)ptr` is an ill-formed
// narrowing cast on LP64. Widened to `(u32)(uintptr_t)` (value-preserving — the
// stored offsets are <4GB, and the difference is identical). Verbatim otherwise.
...
J3DMaterialFactory::J3DMaterialFactory(const J3DMaterialBlock& block)
{
	...
	if (block.mpIndInitData != nullptr
	    && ((u32)(uintptr_t)block.mpIndInitData
	        - (u32)(uintptr_t)block.mpNameTable)
	           > 4)
