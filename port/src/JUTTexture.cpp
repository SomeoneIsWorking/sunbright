// Owned portable copy of reference/sms/src/JSystem/JUtility/JUTTexture.cpp.
// Identical to the decomp EXCEPT the two 64-bit pointer-truncation sites in
// JUTTexture::storeTIMG: `(int)mTexInfo + offset` casts a ResTIMG* down to int
// (4 bytes) before re-forming a void*, which is lossy on LP64 (x86-64/arm64)
// and a hard error without -fpermissive. Routed through uintptr_t so the
// pointer arithmetic happens at full pointer width. No masking intent here (the
// GC code just adds an offset to the header pointer), so no mask is applied.
...
void JUTTexture::storeTIMG(ResTIMG const* param_0)
{
	if (param_0) {
		mTexInfo = param_0;
		mTexData = (void*)((uintptr_t)mTexInfo + mTexInfo->imageDataOffset);
