// OWNED COPY of reference/sms/src/JSystem/JAudio/JASystem/JASSeqParser.cpp —
// keep in sync. ONLY 64-bit-port change: cmdPrintf's local `registers[4]` holds
// either an int register value OR, for a `%s` arg, a host string pointer
// (`(int)&mRawFilePtr[...]`). Widened the array (and that cast) to intptr_t so
// the pointer is not truncated on LP64; the int values widen harmlessly. (The
// OSReport that consumes them is compiled out in release, but the cast is fixed
// for correctness regardless.) Verbatim otherwise.
#include <stdint.h>
...
int TSeqParser::cmdPrintf(TTrack* track, u32* args)
{
	char buffer[128];
	u8 byteArray[4];
	intptr_t registers[4]; // 64-bit port: may hold a host string ptr (%s case)
	u32 count = 0;
