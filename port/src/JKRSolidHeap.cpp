// OWNED COPY of reference/sms/src/JSystem/JKernel/JKRSolidHeap.cpp — keep in sync.
// 64-bit-port changes: (1) allocFromHead/allocFromTail align the host bump
// pointers mCurStart/mCurEnd via ALIGN_NEXT/ALIGN_PREV((u32)ptr,...) — widened
// the cast to uintptr_t so the pointer is not truncated on LP64 (result casts
// back to char*). (2) state_register's `checkCode` is a STATE CHECKSUM, not an
// address — kept as u32 but its pointer inputs are widened via uintptr_t before
// being folded in (the low 32 bits stay the checksum). Verbatim otherwise.
#include <stdint.h>
