...
// reference/sms/src/JSystem/JAudio/JASystem/JASPlayer_impl.cpp.
//
// Identical to the decomp EXCEPT two C++11 brace-init narrowing sites: the
// oscillator vibrato/tremolo tables (`s16 sVibTable[]`, `s16 sTreTable[]`) hold
// the bit patterns 0xC000 (49152) and 0x8001 (32769), which are out of range
// for a *signed* 16-bit field. CodeWarrior accepted the implicit wrap; ISO C++
// rejects it under -Wnarrowing. The intended values are the two's-complement
// negatives (-16384 / -32767), so the fix is an explicit (s16) cast on those
// literals — bit-identical, behaviour-preserving. Nothing else changed.
...
namespace Player {
	...
	s16 sVibTable[18] = {
		...
		0x0000, 0x000C, (s16)0xC000, 0x0000, 0x000C, 0x0000, 0x000D, 0x0000, 0x0001,
	};
	s16 sTreTable[18] = {
		0x0000, 0x0000, 0x7fff, 0x0000, 0x0014, 0x0000, 0x0000, 0x0014, (s16)0x8001,
