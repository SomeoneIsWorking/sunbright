---
id: 22
title: GX compatibility renderer omits GXCopyTex post-copy EFB clear
status: resolved
...
tags: render,recomp,gx-compat,efb-copy,clear,parity
created: 2026-08-27
updated: 2026-08-28
---
...
The FIFO parser received the BP `0x52` copy trigger, but the typed handoff into the GX compatibility renderer
...
behaviour, and the pre-copy/post-copy epoch boundary. A live GX-compatibility/Aurora parity run is still
required before attributing any whole-frame score change to this fix.

### Reopened (2026-08-28)
2026-08-28 live GX-compatibility run: Vulkan validation VUID-vkCmdDraw-None-04007 on every post-copy synthetic clear. Root cause is broader than the clear batch: render_pass binds g_vbuf once, then GXCopyTex ends the render pass and begins a resumed pass without rebinding vertex input state. The clear batch became the first draw after that resume and exposed the invalid command. Reopen until every render-pass begin shares one binding path and a guarded live control is clean.

### Resolution (2026-08-28)
The typed GXCopyTex clear path and render-pass rebind fix pass CPU controls. A guarded live GX-compatibility/Aurora A/B run on 2026-08-28 exercised ordered EFB copies through 130 presents, produced no VUID-vkCmdDraw-None-04007 or other Vulkan validation error, and shut down GPU-clean.
