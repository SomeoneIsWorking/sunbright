...
Both runtimes now copy the camera matrix from JDrama::TGraphics at the high-level TViewObj::testPerform draw-dispatch funnel. Perspective, orthographic, and pre-camera/empty transitions are explicit and tested; recomp no longer reads sbr_gx_current_projection and native decomp no longer calls GXGetProjectionv. Guarded fastboot runs submitted 6,468 recomp and 11,858 decomp models and exited 0 while retaining the original dispatch/draw bodies.
