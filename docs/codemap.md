...
| HW devices | ✅ | `sms-recomp/runtime/devices/` (gxfifo, vi, ai, si, exi, dsp, aram, mi, pi, di, sram) | `dev_gxfifo.cpp` frames GX and hands it to Aurora. `gx_fifo_input.{h,cpp}` owns write-gather reassembly: direct big-endian 1/2/4-byte appends and cursor consumption replace 123k generic vector insertions per settled plaza frame. `gx_fifo_vertex_layout.{h,cpp}` owns the VCD/VAT vertex-size rule (including NBT3) with direct/indexed known-difference controls. `gx_fifo_2d.{h,cpp}` owns the orthographic-draw decoder, gate telemetry, and completeness report; the formatted root device is 1,603 lines, below its 1,968-line legacy ratchet. The controlled work reporter shows ~357 KB guest FIFO input expanding through display lists to ~1.80 MB of Aurora input, with zero compaction/capacity growth after warmup |
...
| Subsystem | Status | Where | Gap / next |
|---|---|---|---|
| GX command processor | 🟡 | `extern/aurora/lib/gx/command_processor.cpp`, `auto_array_sizing.hpp`, `extern/aurora/lib/gx/fifo.cpp` | replay + native emission. Auto-array maximum-index scanning caches the indexed layout per VAT and specializes exact 1–6-field loops (including flattened NBT3); five controlled CPU tests cover mixed widths, NBT3, the >6 fallback, empty input, and a changed-index known difference. `SB_DRAW_STATS` reports actual synchronous replay calls/bytes and deterministic scan work even though the live FIFO is empty at drain |
