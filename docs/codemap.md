...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | 🟡 Audit/preview renders rigid and multi-matrix J3D meshes for the supported solid-colour, textured, masked, diffuse, and specular material families plus ordered pane/immediate pictures, resource-font glyphs, complete J2D windows, and filled rectangles; `./run.sh --semantic-preview` gives that incomplete target sole ownership of the live window while Aurora stays offscreen |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | 🟡 The same audit and explicit visible-preview modes consume the shared mesh/material/pose contract through native-layout adapters plus the picture, glyph, J2D window, filled-box, and GC2D fill stream; broader 3D materials and effects remain absent |
...
| PC-native semantic renderer | 🟡 | public schema/API and shared model/material/image/J3D decoders plus `model_context.*`, `j3d_fog.*`, `j3d_lit_material.*`, `j3d_specular_material.*`, `j3d_tinted_layered_material.*`, `j3d_masked_toon_material.*`, `j3d_alpha_masked_material.*`, `j3d_lit_alpha_mask_material.*`, `j3d_stage_lighting.*`, and `particle_billboard.*`: `native-render/include/`, `native-render/src/`; ordered `SemanticDraw` frame/sink, 2D implementations, `semantic_3d_pass.cpp`, shared `sdl_image_cache.cpp`, and `sdl_semantic_frame_client.cpp`: `native-render/src/`; PC shaders and controls: `native-render/shaders/`, `native-render/tests/`; recomp J3D model/material/light/guest-camera adapters and high-level matrix-object capture: `sms-recomp/overrides/{semantic_j3d_*,j3d_scene_projection_adapter.*,guest_j3d_texture_adapter.*,mtx_crosscheck.cpp}` with the sole shared `TViewObj::testPerform` hook composed in `sms-recomp/frame_interp/subframe_legacy.cpp`; recomp particle adapter: `sms-recomp/overrides/semantic_particle_adapter.{h,cpp}` at the retained `JPADrawExec::exec` wrapper; recomp J2D adapters remain under `sms-recomp/overrides/`; decomp J3D and particle adapters: `sms-boot/runtime/native_j3d_*`, `sms-boot/runtime/native_particle_adapter.cpp`, `sms-boot/shims/{native_j3d_lighting_bridge.h,sb_native_particle.h}`, plus the high-level camera and stage-light hooks in `decomp/sms/src/JSystem/JDrama/JDRViewObj.cpp` and `decomp/sms/src/MarioUtil/LightUtil.cpp`; decomp J2D adapters: `sms-boot/runtime/native_{jut_texture,picture,window,resource_font,j2d_fill_box,solid_rectangle}_adapter.cpp`; layout adapters are owned as `sms-native-semantic`, while the optional decomp lighting callback is implemented directly by `sms-boot`; composition/frame seams remain in each runtime | `semantic_sink` owns the exclusive producer lease and `semantic_frame_bridge` owns begin/seal lifecycle. `Semantic3dPass` owns PC-native 3D submission and runs before `Semantic2dPass` overlays the ordered UI. One bounded immutable-revision image cache implementation serves both passes. Runtime adapters feed copied values only and retain both original bodies. Each mesh vertex may select a compact index from the draw's bounded view-space pose; runtime-specific J3D slots and object layouts stop in the adapters. Camera projection and stage lights come from high-level game owners; the lighting boundary carries normals, material/vertex colour choice, ambient, up to two view-space point lights with authored distance falloff, and the primary light's directional-specular direction, colour, and shininess. The material boundary also carries high-level cull, depth, alpha-cutout, straight/premultiplied-alpha blend, linear view-depth fog, animated tint, independently sampled two-texture layers, four-image masked toon surfaces, solid-colour texture-mask values, and an independent second-UV alpha mask paired with its colour texture, plus the first exact standard JPA billboard family and explicit destination-alpha blending. Neither model adapter consumes GX SDK/FIFO/compatibility state; the recomp matrix-load trace is an independent assertion-only control. PC-native product semantics belong here, never in the GX compatibility renderer. |
| J3D semantic texture binding | 🟡 | shared bounded J3D capture plus slot resolution and primary/secondary UV selection: `native-render/include/sunbright/native_render/{j3d_material_state,model}.h`, `native-render/src/{j3d_unlit_material,model}.cpp`; layout capture: `sms-recomp/overrides/semantic_j3d_material_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | Runtime adapters preserve all eight texture bindings, all sixteen colour-stage records, and all three programmable colour registers independently of active stage count, then resolve only the selected slots into decoded images for a supported semantic family. The renderer-neutral material carries images and selected UV sets, never J3D slot numbers or console stages. |
| J3D semantic specular materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_specular_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; layout adapters: `sms-recomp/overrides/semantic_j3d_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | Exact reached texture-free and textured J3D programs are reduced above GX to affine diffuse, optional texture, additive-colour, and directional-specular terms plus the material/vertex colour source. Both runtime adapters call the same classifier; no console colour stage crosses the renderer interface. |
| J3D semantic layered materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_layered_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; shipping shader: `native-render/shaders/model_layered_lit.frag.glsl`; layout adapters: `sms-recomp/overrides/semantic_j3d_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | Two independently decoded textures and UV sets combine with high-level signed diffuse lighting through one explicit detail weight. Runtime adapters own J3D slot/order layout; the shared renderer owns the ordinary weighted image equation and never receives console stages. |
| J3D semantic tinted layered materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_tinted_layered_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; shipping shader: `native-render/shaders/model_tinted_layered.frag.glsl`; layout adapters: `sms-recomp/overrides/semantic_j3d_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | Two decoded images and UV sets combine through two ordinary clamped layers with animated colour, signed diffuse from the authored one- or two-light context, directional specular, material alpha, and linear fog. Runtime adapters own J3D layout; the shared renderer never receives console selectors or register identities. |
| J3D semantic masked toon materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_masked_toon_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; shipping shader: `native-render/shaders/model_masked_toon.frag.glsl`; layout adapters: `sms-recomp/overrides/semantic_j3d_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | Four decoded image chains and four UV sets implement the reached Mario surface: 8-bit hand-mask selection between main and alternate image, a weighted toon-ramp/diffuse layer, and authored static plus directional highlights. Runtime adapters own J3D layout; the shared renderer receives only ordinary image, lighting, colour, fog, and raster values. |
| J3D semantic effect materials | 🟡 | shared classifiers and renderer-neutral materials: `native-render/{include/sunbright/native_render,src}/j3d_effect_material.*`, `j3d_dual_alpha_effect_material.*`, `model.*`; shipping pipeline: `native-render/src/semantic_3d_pass.cpp`; layout adapters: `sms-recomp/overrides/semantic_j3d_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | The reached lens-glow families reduce exact primary-light programs to decoded texture modulation: authored K0 RGB, scaled K0 texture terms, register-0 RGB, white passthrough, texture-driven interpolation between K0 and register-0 RGB, or the dedicated two-texture dual-alpha family. The latter combines two sampled alpha inputs, primary-light diffuse RGB, and an authored additive `2 * C0 * C1` colour term. They preserve authored blend and alpha-test policies. Unsupported effect programs remain on the retained renderer; the semantic boundary carries no TEV or register identity. |
| J3D semantic lit alpha-tint material | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_lit_alpha_tint_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; shipping shader: `native-render/shaders/model_lit_alpha_tint.frag.glsl`; layout adapters: `sms-recomp/overrides/semantic_j3d_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp`; pass wiring: `native-render/src/semantic_3d_pass.cpp` | The reached one-stage hand program uses one decoded texture for alpha only, multiplies primary diffuse lighting by the authored register-0 RGB tint, and multiplies texture alpha by material alpha. The shared boundary carries only image, tint, ordinary lighting, and raster policy; no TEV or register encoding survives. |
