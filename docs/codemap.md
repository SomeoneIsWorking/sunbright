...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | 🟡 Shared semantic `J2DPicture` command/layout plus tested guest adapter are wired before the retained body; an independent SDL3 pass is GPU-verified offscreen but not installed in the live frame |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | 🟡 Native `J2DPicture` fields feed the same semantic command/layout before the retained body; the shared SDL3 pass is verified offscreen but not installed in the live frame |
...
| host entry | ✅ | `sms-recomp/host/main.cpp`, `sms-recomp/host/aurora_config.{h,cpp}`, `sms-recomp/host/dol_loader.{h,cpp}` | `main.cpp` composes settings, hidden UI runtime, Aurora, disc, memory, and guest execution; the game is the unconditional startup surface. `aurora_config` owns the tested renderer launch policy, including the INFO default that prevents per-command debug logging from becoming frame work. `dol_loader` owns DOL parsing/install and arena-low derivation |
...
| SDL3-GPU GX compatibility renderer | 🟡 | `sms-recomp/runtime/render/native_render.cpp`, `native_presenter.{h,cpp}`, `native_gpu_guard.{h,cpp}`, `native_gpu_admission.{h,cpp}`, `native_gpu_pipeline.{h,cpp}`, `native_raster_state.h`, `native_efb_copy_plan.{h,cpp}`, `native_efb_copy_clear_draw.{h,cpp}`, `native_tev_uniform.{h,cpp}`, shaders, `scene.cpp`, `state_oracle.cpp` | Owns the diagnostic FIFO→GX-state→SDL3-GPU implementation, exact Aurora comparison, and its safety boundary. The legacy selector is `SBR_RENDERER=native`; UI and documentation call it GX Compatibility. Do not add new PC-native product semantics here. |
| PC-native semantic renderer | 🟡 | public schema/API: `native-render/include/`; semantic implementations, fixed-depth J2D context stack, renderer-neutral asset decoder, bounded frame collector, and independent SDL3 picture pass: `native-render/src/`; PC shaders and tracked generated binaries: `native-render/shaders/`; CPU/GPU controls: `native-render/tests/`; recomp guest byte owner, layout decoder, context override: `sms-recomp/overrides/guest_byte_reader.{h,cpp}`, `j2d_picture_adapter.{h,cpp}`, `semantic_j2d_context.{h,cpp}`; leaf submission remains in the sole picture override owner `diag_2d.cpp`; decomp layout/context adapter: `sms-boot/runtime/native_picture_adapter.cpp` and `sb_native_picture_*` seams | The first J2D picture value path is wired in both runtimes without FIFO/BP/XF/TEV/EFB types. `image_decode.{h,cpp}` converts all eleven tiled game-asset image encodings and all three palette encodings into ordinary RGBA8 with exact-range validation and content revisions; it has no guest-memory or GPU ownership. Both layout adapters capture and decode exact JUT texel/palette bytes at draw entry, then atomically submit each semantic command with its matching image views. Each retained `J2DScreen::draw` now scopes a copied logical ortho rectangle, physical viewport, and clip-enable value; the picture leaf supplies final hierarchy clip and append order. `frame.{h,cpp}` owns a complete bounded multi-canvas semantic frame, copies transient pixels, preserves order, and rejects conflicting identities and capacity/lifecycle errors. The semantic SDL3 pass has guarded full-canvas/sub-viewport known-different GPU controls. Next ownership: install the collector at the exact frame seams and add a shared host device/presenter, while expanding the order stream beyond pictures before bypassing GX. Future scene, material, lighting, particle, and effect families belong under `native-render/`, never the GX compatibility renderer. |
| 2D / HUD overrides | 🟡 | `sms-recomp/overrides/diag_2d.cpp`, `j2d_picture_adapter.{h,cpp}`, `sms-recomp/overrides/hud.cpp` | `diag_2d.cpp` remains the sole `0x802cc7c0` registration owner and publishes semantic state at entry before always super-calling the recompiled body. Its older post-body `SBR_J2D_CAPTURE` path belongs only to GX compatibility. `hud.cpp` owns current widescreen corner layout. |
