...
| J3D semantic texture binding | 🟡 | shared slot resolution and primary/secondary UV selection: `native-render/include/sunbright/native_render/{j3d_material_state,model}.h`, `native-render/src/{j3d_unlit_material,model}.cpp`; layout capture: `sms-recomp/overrides/semantic_j3d_material_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | Texture binding slots are captured independently of active colour-stage count. Runtime adapters resolve the selected slot to one decoded image; the renderer-neutral material carries that image and its selected UV set, never J3D slot numbers. |
