...
| native renderer (SDL3 GPU) | 🟡 | `sms-recomp/runtime/native_render.cpp` (+ `sms-recomp/runtime/shaders/`, `sms-recomp/runtime/gx_texture.cpp`) | geometry, per-material depth/blend, textures, TEV, alpha test, wrap modes. edgeIoU 25.2% / lumaCorr +0.538 vs the aurora oracle. KNOWN GAP: stages pinned to texture unit 0 (`SBR_TEXMAP_NAMED=1` opts in, currently worse — cause narrowed, not found). See `debug_journal/2026-07-23_native_texgen_and_texmap_bisect.md` |
