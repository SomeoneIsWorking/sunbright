...
| File-select / save screen | 🟡 | **CLOSE, not yet confirmed pixel-parity (2026-07-15)** vs the TRUE Dolphin framedump. Water MATCHES (ΔABS 3.2). Under-lighting FIXED. But a full-frame compare (`scratch/shots/fsel_vs_dolphin.png`) shows unconfirmed residuals: palm-trunk paler in native (Δ35), Mario pose+paleness differ, and horizontal framing/layout differs (OPTIONS cut off — likely camera-PAN-position/capture-timing since the file-select camera pans, but NOT confirmed). Must align both captures to the same camera-pan state before judging, and take a real look at the palm-trunk paleness. Earlier "PARITY COMPLETE" was premature (verified water numerically, eyeballed the rest). Palm + A/B/C cubes under-lighting FIXED (the missing `TLightCommon::setLight` tail `GXSetChanAmbColor(GX_COLOR0A0, getAmbColor(idx))` — global ambient was stuck at the `ReInitializeGX` black; map objects are faithfully LightOff so they depend on it; fix also restores ambient for ALL lit scenes). Ocean sun-glare was NOT a native defect — an aurora `SB_FIFO_REPLAY` water artifact; the TRUE Dolphin water is turquoise `[96,188,201]` and native matches `[96,193,204]`. Verified via fork `dolphin-emu-nogui -p headless -e ...dff -C Movie.DumpFrames` (`scratch/shots/dolphin_fsel_true.png`). Follow-ups (separate, non-blocking): ambient ALPHA=0 vs 0xff (getAmbColor field nuance, non-visible); aurora `.dff`-replay mis-renders EFB-copy water (diagnostic-tool bug, unreliable oracle for mirror/feedback scenes). Detail: `debug_journal/2026-07-15_fileselect_oracle_parity.md` |
