...
| PC-native semantic renderer | 🟡 | public schema/API: `native-render/include/`; semantic implementations, renderer-neutral asset decoder, and independent SDL3 picture pass: `native-render/src/`; PC shaders and tracked generated binaries: `native-render/shaders/`; CPU/GPU controls: `native-render/tests/`; recomp layout adapter: `sms-recomp/overrides/j2d_picture_adapter.{h,cpp}`; decomp layout adapter: `sms-boot/runtime/native_picture_adapter.cpp` and the `sb_native_picture_submit` seam | The first J2D picture value path is wired in both runtimes without FIFO/BP/XF/TEV/EFB types. `image_decode.{h,cpp}` converts all eleven tiled game-asset image encodings and all three palette encodings into ordinary RGBA8 with exact-range validation and content revisions; it has no guest-memory or GPU ownership. Both layout adapters capture and decode exact JUT texel/palette bytes at draw entry, then atomically submit each semantic command with its matching image views. The semantic SDL3 pass has guarded known-same/known-different GPU controls. Next ownership: add J2D canvas/clip/order context, a bounded frame collector, and a shared host device/presenter. Future scene, material, lighting, particle, and effect families belong under `native-render/`, never the GX compatibility renderer. |
