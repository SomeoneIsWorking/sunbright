...
| J3D semantic specular materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_specular_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; layout adapters: `sms-recomp/overrides/semantic_j3d_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | Exact reached J3D programs are reduced above GX to affine texture/diffuse, additive-colour, and directional-specular terms plus the material/vertex colour source. Both runtime adapters call the same classifier; no console colour stage crosses the renderer interface. |
