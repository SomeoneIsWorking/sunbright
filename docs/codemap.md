...
| host settings + RmlUi | 🟡 | `sms-recomp/app/`, `sms-recomp/ui/`, `res/rml/`; behavior map: `docs/app/settings.md` | Dusklight ownership split: typed persisted policy is independent of RmlUi; the prelaunch document selects Aurora/native-preview and frame cadence. Headless control validates 7/7 nonzero controls and viewport-contained layout. Interpolated Unlocked is shown unavailable because Aurora replay owns one mutable-history midpoint; native renderer remains an offscreen parity preview, not the displayed picture |
...
| **frame cadence / interpolation** | 🟡 | **`sms-recomp/app/frame_rate.{h,cpp}`** owns user policy; **`sms-recomp/frame_interp/`** owns the capped interpolation mechanism. Map: **`docs/60fps/README.md`**, UI contract: **`docs/app/settings.md`** | Vanilla, Interpolated 60, Native 60, and Native Unlocked are wired. Native modes override `JDrama::TVideo::waitForRetrace`'s game-requested field count to one; Native Unlocked also removes host pacing. Interpolated 60 remains the measured stream-replay path. **Interpolated Unlocked is not implemented and is refused, never coerced to capped**: Aurora captures one replay snapshot, mutates interpolation history while producing exactly one midpoint, then consumes the snapshot. The proper next step is a reusable read-only interpolation plan plus display-timed multi-emission replay. Existing per-population coverage and residuals remain documented in `docs/60fps/README.md` |
