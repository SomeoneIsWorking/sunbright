...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | 🟡 Explicit audit mode renders ordered pane/immediate pictures, resource-font glyphs, complete J2D windows, generic J2D filled/gradient boxes, and `GC2D fill_rect` values offscreen through SDL GPU; Aurora remains visible and semantic presentation is not yet owned |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | 🟡 Explicit audit mode feeds the same native picture, glyph, J2D window, filled-box, and GC2D fill values through the ordered offscreen client; Aurora remains visible and semantic presentation is not yet owned |
...
| PC-native semantic renderer | 🟡 | public schema/API: `native-render/include/`; ordered `SemanticDraw` frame/sink plus picture, resource-font glyph, solid-rectangle, and J2D-window composition implementations, fixed-depth J2D context stack plus active immediate canvas/scissor, renderer-neutral asset decoder, process bridge, `semantic_2d_pass.cpp`, and `sdl_semantic_frame_client.cpp`: `native-render/src/`; PC shaders and controls: `native-render/shaders/`, `native-render/tests/`; recomp picture/text/window adapters: `sms-recomp/overrides/{diag_2d,guest_jut_texture_adapter,j2d_picture_adapter,j2d_window_adapter,resource_font_glyph_adapter,semantic_j2d_context,semantic_j2d_window,semantic_resource_font_glyph}.{h,cpp}`; recomp J2D fill adapter/seam: `sms-recomp/overrides/{j2d_fill_box_adapter,semantic_j2d_fill_box}.{h,cpp}`; recomp GC2D solid adapter/seam: `sms-recomp/overrides/solid_rectangle_adapter.{h,cpp}` and `widescreen_effects.cpp`; decomp adapters: `sms-boot/runtime/native_{jut_texture,picture,window,resource_font,j2d_fill_box,solid_rectangle}_adapter.cpp`; decomp high-level context owner: `sms-boot/runtime/native_j2d_context.{h,cpp}`; the decomp adapters are owned as the `sms-native-semantic-2d` library; composition/frame seams remain in each runtime | `semantic_sink` owns the exclusive producer lease and `semantic_frame_bridge` owns begin/seal lifecycle. `Semantic2dPass` walks pane/immediate pictures, font glyphs, window contents/frame pieces, generic filled/gradient boxes, and GC2D fills in exact submission order inside one render pass. `window.cpp` is the sole window-layout owner and composes existing rectangle/picture commands rather than adding a GX-shaped shader. `SdlSemanticFrameClient` owns its target/fenced submission/readback and never claims or presents a window. Runtime adapters feed copied values only and retain both original bodies. Future visible composition, scene, material, lighting, particle, and effect families belong under `native-render/`, never the GX compatibility renderer. |
