...
| 60fps — record-and-replace path | 🟡 | `sms-recomp/overrides/interp60_replace.cpp` | `SBR_INTERP60_REPLACE=1`, modelled on dusklight's `src/dusk/frame_interpolation.{h,cpp}`. The sim tick runs UNTOUCHED; every model's final draw + normal matrices are recorded at `J3DModel::viewCalc`, keyed by **(model, index)** — NOT the matrix address, because `viewCalc` begins with `swapDrawMtx()` so a joint alternates between two addresses on consecutive ticks; the sub-frame writes `lerp(prev,cur,alpha)` into the live buffer, draws, presents, and restores byte-exactly AFTER the present (the FIFO fetches matrices at copy/present, not at the perform). Guest state is never used as an INPUT to game code, so it cannot leak: cross-run leak test is **0.000% of main-frame pixels** vs 99.91% for the substitution path, `CLOBBERED mid-draw 0` over 750k matched models. VERIFIED 2026-08-06 with a moving camera (65 u/tick): asymmetry is monotone in alpha (−41.0%, +24.0%, +99.2% [mad] at 0.0/0.5/1.0) and the alpha=1 endpoint reaches `sub->next` 3.2% px / mad 0.150. RESIDUAL, measured and NOT yet attributed: at alpha=0 the sub-frame is 36.7% px / 4.41 from the preceding main frame against a full tick of 11.42, and the error is roughly proportional to (1-alpha). It is a PARTIAL response of covered geometry, NOT a frozen population — per-region lead at alpha 0.0/0.5/1.0: sky 0.009/0.491/1.000 (tracks alpha exactly), ground 0.304, sea 0.346, buildings 0.457, uncovered J2D subtitle 0.854/0.874/1.000. Claim C031; C029 said 'frozen content' and is FALSIFIED. FALSIFIED with controls: the 37% of zero-delta matrix elements are invisible slots (`SBR_INTERP60_REPLACE_KICK_ONLY=frozen` moves 0.00% of pixels, `=moving` moves 96.33%, exhaustive), and the prev/cur pairing is correctly one tick apart (alpha −1.0 and 2.0 both score worse against the previous main frame than 0.0). The camera pose lerp is nearly inert here (~6 of 41 points). NOT covered, and the report says so every time it prints a number: projection matrix, texture/bump matrices, J2D/ortho, JPA particles, immediate-mode geometry, and any model whose joint count changed. Journal `debug_journal/2026-08-06_motion_census_and_uncovered_residual.md` |
| 60fps — motion census (liveness) | ✅ | `sms-recomp/overrides/interp60_replace.cpp` `sbr_i60r_census` | `SBR_INTERP60_CENSUS=1`, usable with NO interpolation at all. Buckets per-tick `|cur-prev|` over every recorded draw matrix's translation — the matrices the hardware reads — so it watches no named object and cannot be blind the way every camera probe in this arc was (`g_camObj` reads 0.000 through all of gameplay; MTXTRACE auto-pins a model under the MIRROR view). Verdict is the `<1e4` bucket, RUN AGAINST BOTH CLASSES: 115,991 with the camera rotating vs 0 with no input; `>=1e4` is excluded as a constant garbage population that runs backwards between them. `interp60_run.sh` prints the window covering the DUMPED MOMENT before any score |
