...
| GPU submit flight recorder | ✅ | `sms-recomp/runtime/gpu_incident_recorder.{h,cpp}`, reader `sms-recomp/tools/gpu_flight_dump_main.cpp` | armed BEFORE Aurora init (`sms-recomp/host/main.cpp`); every queue BEGIN/RETURN/COMPLETE (+ DEVICE_LOST) lands in a fixed-size pwrite ring under `scratch/gpu_crash/*.flight`, so an abort cannot erase the last submit. After a device loss, `build-sms-recomp/gpu_flight_dump <file.flight>` names the in-flight submit: API-PENDING = died inside the host queue call, GPU-PENDING = accepted, never completed (ring hang). Aurora reports through `AuroraConfig::gpuProbeCallback`. Controls: fork-and-abort survival, corrupt/torn rejection, wrap, stale identity. Verified 2026-08-25: 150-present run -> 450/450 records, 0 corrupt |
...
headless or windowed, and asks the kernel afterwards whether the run disturbed the card).
