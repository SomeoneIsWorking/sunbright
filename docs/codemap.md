...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | 🟡 Explicit audit mode renders ordered `J2DPicture` and `GC2D fill_rect` values offscreen through SDL GPU; Aurora remains visible and semantic presentation is not yet owned |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | 🟡 Explicit audit mode feeds native pictures and `GC2D fill_rect` values through the same ordered offscreen client; Aurora remains visible and semantic presentation is not yet owned |
