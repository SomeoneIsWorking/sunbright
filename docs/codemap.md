...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | 🟡 Explicit audit mode activates the bridge and renders collected `J2DPicture` values offscreen through SDL GPU; Aurora remains visible and semantic presentation is not yet owned |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | 🟡 Explicit audit mode feeds native `J2DPicture` values through the same offscreen client; Aurora remains visible and semantic presentation is not yet owned |
...
| PC-native semantic renderer | 🟡 | public schema/API: `native-render/include/`; semantic implementations, fixed-depth J2D context stack, renderer-neutral asset decoder, bounded frame collector, process bridge, `sdl_semantic_frame_client.cpp`, shared SDL GPU platform/presenter/targets, and independent picture pass: `native-render/src/`; PC shaders and controls: `native-render/shaders/`, `native-render/tests/`; recomp composition: `sms-recomp/host/render_composition.{h,cpp}` and exact seam in `sms-recomp/overrides/native_frame.cpp`; decomp composition: `sms-boot/runtime/semantic_render.{h,cpp}` and exact seam in `sms-boot/runtime/frame_seam.cpp`; runtime-local picture adapters remain at their existing override/native seams | `picture_sink` owns the exclusive producer lease and `semantic_frame_bridge` owns begin/seal lifecycle. `SdlSemanticFrameClient` activates the bridge, consumes each sealed sequence once, owns its target/pass/fenced submission/readback, and never claims or presents a window. `sdl_gpu_platform` owns one process SDL device and an optional sole presenter; device-only clients coexist with Aurora's independent visible WebGPU path. Runtime composition pauses Aurora pipeline compilation around SDL device creation, consumes immediately after seal, and destroys semantic GPU resources before Aurora. Runtime adapters feed copied values only. Future text/window/fill ordering, scene, material, lighting, particle, and effect families belong under `native-render/`, never the GX compatibility renderer. |
