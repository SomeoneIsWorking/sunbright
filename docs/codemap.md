...
| **60fps interpolation (ALL of it)** | 🟡 | **`docs/60fps.md`** — the map | THREE independent implementations, no shared code or vocabulary: **A** stream interpolation (`SBR_60FPS`, `runtime/lerp60.{h,cpp}` -> `aurora::gfx::interp`) rewrites the RECORDED frame's matrices and presents twice, runs no game code in the sub-frame, and is **the only one whose EFB feedback, camera cuts and screen-sampling effects work**; **B** substitute-and-re-issue (`SBR_INTERP60`, `overrides/interp60_snapshot.cpp`, 2951 lines) writes an interpolated pose into guest objects and re-runs the draw lists; **C** record-and-replace (`SBR_INTERP60_REPLACE`, `overrides/interp60_replace.{h,cpp}`, dusklight's model) lerps each J3DModel's final draw matrices, keyed **(model, index)** because `viewCalc` begins with `swapDrawMtx()`. C is what `play.sh --60fps` selects and it covers J3DModel matrices ONLY — HUD, particles, immediate-mode geometry and every screen effect step at 30Hz inside a 60Hz frame, which is the flicker. **Every hook, by guest address, is tabulated in `docs/60fps.md`** (9 for A, 4 for B, 0 of C's own — C cannot run without B). 49 env switches; ~24 are investigation diagnostics that belong behind `SB_LOG`. UNIFICATION TARGET, also in that file: one `sms-recomp/frame_interp/` module shaped like dusklight's `src/dusk/frame_interpolation.{h,cpp}` — A's mechanism, C's pairing, effects opting in via `add_interpolation_callback`, `request_presentation_sync()` for frames that must be exact, one setting carrying dusklight's `FrameInterpMode` enum (off / 60 / unlimited) |
