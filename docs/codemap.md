...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | 🟡 Explicit audit mode renders ordered pane/immediate pictures, resource-font glyphs, generic J2D filled/gradient boxes, and `GC2D fill_rect` values offscreen through SDL GPU; Aurora remains visible and semantic presentation is not yet owned |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | 🟡 Explicit audit mode feeds the same native picture, glyph, J2D filled-box, and GC2D fill values through the ordered offscreen client; Aurora remains visible and semantic presentation is not yet owned |
...
| host entry | ✅ | `sms-recomp/host/main.cpp`, `sms-recomp/host/render_composition.{h,cpp}`, `sms-recomp/host/aurora_config.{h,cpp}`, `sms-recomp/host/dol_loader.{h,cpp}` | `main.cpp` composes settings, hidden UI runtime, Aurora, render clients, disc, memory, and guest execution; the game is the unconditional startup surface. `render_composition` owns the mutually exclusive GX-compatibility and semantic-audit policies, pauses Aurora pipeline compilation only around SDL device creation, and unwinds semantic collection, the active Aurora frame, UI, renderer resources, then Aurora. `aurora_config` owns Aurora launch policy; `dol_loader` owns DOL parsing/install and arena-low derivation |
...
| PC-native semantic renderer | 🟡 | public schema/API: `native-render/include/`; ordered `SemanticDraw` frame/sink plus picture, resource-font glyph, and solid-rectangle implementations, fixed-depth J2D context stack plus active immediate canvas/scissor, renderer-neutral asset decoder, process bridge, `semantic_2d_pass.cpp`, and `sdl_semantic_frame_client.cpp`: `native-render/src/`; PC shaders and controls: `native-render/shaders/`, `native-render/tests/`; recomp picture/text adapters: `sms-recomp/overrides/{diag_2d,j2d_picture_adapter,resource_font_glyph_adapter,semantic_j2d_context,semantic_resource_font_glyph}.{h,cpp}`; recomp J2D fill adapter/seam: `sms-recomp/overrides/{j2d_fill_box_adapter,semantic_j2d_fill_box}.{h,cpp}`; recomp GC2D solid adapter/seam: `sms-recomp/overrides/solid_rectangle_adapter.{h,cpp}` and `widescreen_effects.cpp`; decomp adapters: `sms-boot/runtime/native_{picture,resource_font,j2d_fill_box,solid_rectangle}_adapter.cpp`; decomp high-level context owner: `sms-boot/runtime/native_j2d_context.{h,cpp}`; the decomp adapters are owned as the `sms-native-semantic-2d` library; composition/frame seams remain in each runtime | `semantic_sink` owns the exclusive producer lease and `semantic_frame_bridge` owns begin/seal lifecycle. `Semantic2dPass` walks pane/immediate pictures, font glyphs, generic filled/gradient boxes, and GC2D fills in exact submission order inside one render pass; glyphs remain a distinct semantic family while both fill producers share one source-labelled rectangle implementation. `SdlSemanticFrameClient` owns its target/fenced submission/readback and never claims or presents a window. Runtime adapters feed copied values only and retain both original bodies. Future window ordering, scene, material, lighting, particle, and effect families belong under `native-render/`, never the GX compatibility renderer. |
...
| frame seam / present | ✅ | `sms-boot/runtime/frame_seam.cpp`, `sms-boot/runtime/semantic_render.{h,cpp}` | `sb_frame_present` in TVideo::waitForRetrace; semantic composition consumes every sealed frame independently of window availability while the host-allocation gate is active, and owns device-only setup/teardown around Aurora. The seam also owns the strict decomp present limit and the same `SB_MAX_PRESENT_HZ` submission ceiling as recomp |
