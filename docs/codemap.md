...
| J3D semantic effect materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_effect_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; shipping pipeline: `native-render/src/semantic_3d_pass.cpp`; layout adapter: `sms-recomp/overrides/semantic_j3d_adapter.cpp` | The reached lens-glow families reduce exact primary-light programs to decoded texture modulation: authored K0 RGB, GX constant-ramp RGB, register-0 RGB, white passthrough, or texture-driven interpolation between K0 and register-0 RGB, with either sampled alpha or an authored constant-alpha replacement. They preserve additive source-alpha/destination-one blending, explicit alpha thresholds, and disabled depth testing where authored. Unsupported effect programs remain on the retained renderer; the semantic boundary carries no TEV or register identity. |
