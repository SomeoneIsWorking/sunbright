...
| Gameplay (Delfino) | ⬜ | boot clears tree init + the GXBegin/GXEnd abort (2026-07-15) → drains the fifo. Next: the per-frame **storage** staging buffer overflows — root-caused to the phase-1 **ghost pass** ~doubling all indexed-array uploads (SB_SKIP_GHOST=1 removes the overflow), compounded by Delfino's real ~16MB single-pass geometry > the 8MB cap. Proper fix = kill the ghost pass (re-verify bit-identical off-title first) THEN right-size the cap — NOT a blind bump. Underneath (SB_SKIP_GHOST=1) lies the true next frontier: a fifo desync `unknown opcode 0x70` at a `StaticMapObj ShadowOpa` draw (a vtx-size mismatch). See `debug_journal/2026-07-15_delfino_storage_overflow_ghost_pass.md`. Still GATED behind the title oracle gate |
