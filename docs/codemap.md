...
| J3D semantic effect materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_effect_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; shipping pipeline: `native-render/src/semantic_3d_pass.cpp`; layout adapter: `sms-recomp/overrides/semantic_j3d_adapter.cpp` | The reached lens-glow families reduce three exact primary-light/one-stage programs to decoded texture modulation: authored K0 RGB, register-0 RGB, or white passthrough, with the authored alpha paths. They preserve additive source-alpha/destination-one blending, explicit alpha thresholds, and disabled depth testing where authored. Unsupported effect programs remain on the retained renderer; the semantic boundary carries no TEV or register identity. |
