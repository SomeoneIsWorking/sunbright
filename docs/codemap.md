...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | 🟡 Shared semantic `J2DPicture` producer and exact frame bridge are wired around retained behavior; the bridge is inactive until host composition owns semantic presentation |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | 🟡 Native `J2DPicture` values and exact frame bridge feed the same renderer-neutral owners; the bridge is inactive until host composition owns semantic presentation |
...
| PC-native semantic renderer | 🟡 | public schema/API: `native-render/include/`; semantic implementations, fixed-depth J2D context stack, renderer-neutral asset decoder, bounded frame collector, process-level semantic frame bridge, shared SDL GPU platform/presenter/targets, and independent SDL3 picture pass: `native-render/src/`; PC shaders and tracked generated binaries: `native-render/shaders/`; CPU/GPU controls: `native-render/tests/`; recomp guest byte owner, layout decoder, context override: `sms-recomp/overrides/guest_byte_reader.{h,cpp}`, `j2d_picture_adapter.{h,cpp}`, `semantic_j2d_context.{h,cpp}`; leaf submission remains in the sole picture override owner `diag_2d.cpp`; exact recomp frame calls: `sms-recomp/host/main.cpp`, `sms-recomp/overrides/native_frame.cpp`; decomp layout/context adapter and exact frame calls: `sms-boot/runtime/native_picture_adapter.cpp`, `sms-boot/runtime/frame_seam.cpp`, and `sb_native_picture_*` seams | `picture_sink.{h,cpp}` owns the exclusive producer lease. `semantic_frame_bridge.{h,cpp}` owns begin/seal lifecycle and remains inert until host composition activates it. `sdl_gpu_platform`, `sdl_gpu_presenter`, and `sdl_gpu_frame_target` own one device/window claim and independent client targets; clients shut down before the platform. `PicturePass` owns semantic shaders, pipelines, and a submit-confirmed image-cache transaction while borrowing command/target/presentation ownership. Runtime adapters own their object layouts and feed only copied values. Future text/window/fill ordering, scene, material, lighting, particle, and effect families belong under `native-render/`, never the GX compatibility renderer. |
