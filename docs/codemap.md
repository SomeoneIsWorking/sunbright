...
| File-select / save screen | 🟡 | **aurora attnFn-decode bug FIXED (replay-verified); live-native Mario still washed = separate game-logic gap (2026-07-15).** ROOT CAUSE of the wash on the retail GX stream: aurora's XF channel-control `attnFn` decode had bit9/bit10 SWAPPED vs GC hardware (`reference/sms` GXLight.c / Dolphin XFMemory.h), so a COLOR1 `GX_AF_SPEC` highlight light was misread as `GX_AF_NONE` → added as a full-white constant via TEV stage 4 (COLOR1A1) → blew Mario toward white. This IS the "additive R,G-lift" signature noted below. Fixed the decode + matching GXSetChanCtrl encode in aurora; **verified on the .dff REPLAY** (overalls [127,164,195] washed → [10,69,175], oracle [19,68,169]; Mario matches oracle). General fix — every GX_AF_SPEC/GX_AF_NONE lit object. Title unaffected. **BUT live-native Mario still washed = FOUND: L2 specular attenuation not set** (fix applied — ch1 decodes SPEC; straps now blue). NOT ambient (main Mario amb=0.5 = retail; amb=1.0 is the mirror pass). Measured vs retail dff: retail L2 `cosAtt=(0,0,1) distAtt=(25,0,-24)` (=GXInitLightShininess 50) but live-native L2 `(1,0,0)/(1,0,0)` constant → SPEC attn→1→full-white add. Located `reference/sms/src/MarioUtil/LightUtil.cpp:281` (GXInitLightAttn constant overwrites the GXInitSpecularDir at :277). FIX (verify-gated): give L2 shininess-50 attn; confirm it's Mario's setLight path + decomp-vs-disasm + live-native re-capture. See journal. See `debug_journal/2026-07-15_mario_paleness_cmpr_falsified.md`. Water speckle = separate replay-only artifact (closed). _(Historical, now superseded:_ TRUE Dolphin oracle now exists (fork `--load-state-at` of `fsel_settled.sav` → framedump AVI; recipe in scratch/oracle/MANIFEST.md; frame `scratch/oracle/loadstate_probe/png/fsel_dolphin_end.png`). Pose-independent regions match the true oracle within ~10-12 levels: water dRGB [-5.7,+9.4,+11.4] (smooth turquoise, **NO speckle**), beach [+9.2,+4.4,+4.1], sky [-2,-12.3,+4.1] — small ~5-10-level native-brighter systematic (same class as the resolved title residual). Under-lighting FIXED (missing `TLightCommon::setLight` tail `GXSetChanAmbColor(GX_COLOR0A0, getAmbColor(idx))`; ambient was stuck at the `ReInitializeGX` black; restores ambient for ALL lit scenes). **WATER ARC CLOSED** — the "white water band" was an aurora `SB_FIFO_REPLAY`/EFB-copy artifact, falsified against the real Dolphin render; do NOT re-open vs `.dff` replays. **OPEN (CONFIRMED defect): Mario overalls washed pale** — native bib RGB [144,166,180] sat 0.18 vs true-oracle navy [20,52,150] sat 0.87. Mario-SPECIFIC (env at parity; 2D dialog blue [40,74,225] is fully saturated). NOT the diffuse-light chain: R,G lifted ~+120 while B ~correct = an ADDITIVE signature (no multiplicative clamp(lighting) can lift matColor's low R/G 7×/3×). LOCALIZED (2026-07-15): every render-state field (matColor/ambient/lights/konst/TEV stages/C-regs/format) matches the oracle EXACTLY per SB_DRAW_DUMP_ALL+SB_TEV_DUMP; the ONLY diff is the CMPR (fmt14) overalls TEXTURE PIXELS — native's BMD CMPR load feeds different bytes than the disc (aurora's decoder is correct: the .dff replay renders navy). NEXT: diff native's bound CMPR bytes vs the .dff; fix the endpoint-byteswap/sub-block-order load bug (also fixes skin/hat, same CMPR path). The earlier 'ambient root cause' was a mirror-reflection-draw red herring — falsified. The 'CMPR texture pixels' conclusion here is FALSIFIED (textures decode byte-identically); the real cause is the attnFn decode swap above._)_ Detail: `debug_journal/2026-07-15_fileselect_oracle_parity.md`, `2026-07-15_mario_paleness_cmpr_falsified.md` |
