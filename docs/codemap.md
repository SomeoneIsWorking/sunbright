...
| host entry | ✅ | `sms-recomp/host/main.cpp`, `sms-recomp/host/dol_loader.{h,cpp}` | `main.cpp` composes settings, UI, Aurora, disc, memory, and guest execution; `dol_loader` owns DOL parsing/install and arena-low derivation |
| host settings + RmlUi | 🟡 | `sms-recomp/app/`, `sms-recomp/ui/`, `res/rml/`; behavior map: `docs/app/settings.md` | Dusklight ownership split: `app/` owns typed persisted policy; `Document → Window → SettingsMenu` owns presentation; `ui::Runtime` routes Escape and blocks at the frame seam while the modal is open. The copied Dusklight window/tab/pane presentation uses its Fira Sans family and CC0 notice. Headless control pushes Escape through SDL, exercises the modal loop, validates 7/7 nonzero controls, and closes through Escape. Interpolated Unlocked remains unavailable; native renderer remains an offscreen parity preview |
...
| **frame cadence / interpolation** | 🟡 | **`sms-recomp/app/frame_rate.{h,cpp}`** owns user policy; **`sms-recomp/frame_interp/`** owns the capped interpolation mechanism. Map: **`docs/60fps/README.md`**, UI contract: **`docs/app/settings.md`** | Vanilla, Interpolated 60, Native 60, and Native Unlocked are wired and can change from the in-game menu. Native modes override `JDrama::TVideo::waitForRetrace`'s requested field count to one; Native Unlocked also removes host pacing. Interpolation no longer caches startup policy: Aurora's host replay override is reversible, and a newly enabled interval snaps its first tick exact before building fresh history. **Interpolated Unlocked is not implemented and is refused, never coerced to capped**: Aurora owns one mutable-history midpoint. The proper next step is a reusable read-only interpolation plan plus display-timed multi-emission replay |
...
- **Interpolated Unlocked** — redesign Aurora replay around a reusable, read-only per-tick interpolation plan, then emit display-timed alphas without advancing pairing history. Do not map it to the capped midpoint path. `docs/app/settings.md`.
