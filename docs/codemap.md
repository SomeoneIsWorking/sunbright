...
| Gameplay (Delfino) | ⬜ | boot clears tree init (TMapObjTree::initMapObj ported 2026-07-15) AND the GXBegin/GXEnd abort (3 decomp-dropped GXEnd() restored, 2026-07-15) → drains the fifo, then aborts DEEPER inside aurora `gfx::push_storage` (.cold/assert branch) via `fifo::push_gx_draw` during `fifo::drain`/`sb_frame_present`. Next boot blocker: that push_storage abort (aurora GX fifo). Loud stubs still pending: `TMapObjTree::perform`, `TGuide::perform`, `TTalk2D2::perform`, etc. Still GATED behind the title oracle gate |
