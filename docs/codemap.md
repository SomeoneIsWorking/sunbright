...
| Subsystem | Status | Where | Gap / next |
|---|---|---|---|
| core dispatch / SPRs / call_ppc | ✅ | `sms-recomp/runtime/rt_core.cpp`, `guest_address_table.h` | SPRs are MACHINE-wide (not per-CPUState) — the locked-cache/THP fix. Recompiled functions and native overrides share a tested sparse direct-address table; the previous hash-miss + binary-search pair consumed 9.65% of profile samples and the replacement 3.96% |
| memory + MMIO | ✅ | `sms-recomp/runtime/devices/mmio.cpp`, `intrinsics.h` | flat MEM1 fast path + locked-cache @0xE0000000; the authoritative range router retains a stable per-thread last-device cache for the write-gather hot path |
...
| **frame cadence / interpolation** | 🟡 | **`sms-recomp/app/frame_rate.{h,cpp}`** owns user policy; **`sms-recomp/frame_interp/`** owns interpolation; Aurora **`extern/aurora/lib/gfx/common.cpp`** and **`indexed_interp.{hpp,cpp}`** own retained replay samples. Map: **`docs/60fps/README.md`**, UI contract: **`docs/app/settings.md`** | All five modes apply at the next tick. Native modes apply BSE's retrace, SMS animation-rate, and ModelGate timing contract before the retail `JDrama::TVideo::waitForRetrace` body, but the current FIFO/render path still misses the 16.67 ms budget in heavy Delfino intervals: direct dispatch and MMIO routing reduced complete ticks from the 22.3 ms baseline to roughly 20.5–21.2 ms, leaving draw parsing, retained-array hashing, and rendering as the leading costs. Interpolated modes retain 30 Hz logic and resample the retained draw plan. Direct vertices, matrices, billboards, camera, and explicitly marked indexed XYZ-f32 arrays are covered; the indexed core has a 0→20 `.25/.5/.75` control, while the idle stage-1 verification scene reached no active `TDLTexQuad` batch, so those two live emitters remain unmeasured rather than falsely green. `SBR_DISPLAY_HZ` is the windowless rate input |
...
| Subsystem | Status | Where | Gap / next |
|---|---|---|---|
| GX command processor | 🟡 | `extern/aurora/lib/gx/command_processor.cpp`, `extern/aurora/lib/gx/fifo.cpp` | replay + native emission; rich SB_* diag toolkit. `fifo::drain` treats synchronous replay and the live FIFO as two submission paths but one end-of-frame accounting boundary, so replayed draw profiles report and reset even when the live buffer is empty |
