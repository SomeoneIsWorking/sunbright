...
| File-select / save screen | 🟡 | reachable (START pans title→save-blocks); renders the full beach scene + UI faithfully vs the Dolphin oracle (2026-07-15, oracle harness = fork `--pad-start-at`). Residuals (2)+(3) **FIXED 2026-07-15**: palm trunk + A/B/C cubes were under-lit because `TLightCommon::setLight` omitted its tail `GXSetChanAmbColor(GX_COLOR0A0, getAmbColor(idx))` — so the global ambient stayed at the `ReInitializeGX` black; the map objects are (faithfully) LightOff so they rely on that global ambient. Now matches oracle (palm/cubes lit; amb-trace emits 0x80/0x28 like the oracle). NOT file-select-specific — fixes ambient for all lit scenes. Remaining: (1) ocean sun-glare/foam missing (reflective-sea gap, dominates the residual red-channel gap); minor: ambient ALPHA=0 vs oracle 0xff (getAmbColor alpha-scale field, secondary/non-blocking). NOT defects: file-block save-data + Mario anim-pose. Recipe + worklist: `debug_journal/2026-07-15_fileselect_oracle_parity.md`. ⚠️ compare at FULL RES — `_640` downscales showed a false white bg |
