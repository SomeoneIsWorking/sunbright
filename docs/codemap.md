...
| native renderer (SDL3 GPU) | 🟡 | `sms-recomp/runtime/render/native_render.cpp`, `native_presenter.{h,cpp}`, `native_gpu_guard.{h,cpp}`, `native_gpu_admission.{h,cpp}`, `native_gpu_pipeline.{h,cpp}`, `native_efb_copy_plan.{h,cpp}`, `native_tev_uniform.{h,cpp}`, `gx_light.{h,cpp}`, `gx_texgen.{h,cpp}`, shaders, `scene.cpp`, `state_oracle.cpp` | Native owns the SDL3-GPU device, Aurora's SDL window, its swapchain, and visible presentation; Aurora presentation is disabled but its offscreen FIFO result remains the oracle. `native_gpu_pipeline` owns shader/pipeline state, `native_tev_uniform` owns the shader-facing TEV layout/packing, and `native_presenter` owns WSI. Initialization and shutdown are transactional and idempotent across every owned GPU resource. Geometry, textures, TEV compare mode, alpha test, wrap, both colour channels, eight texmaps, scissor, and ordered EFB copies are wired. `SBR_RENDERER=native` plus the `run-render.sh`/in-process accident gate selects it. Safety is fail-closed on the first fault, bounded waits, explicit EFB-copy source validation, transactional resource replacement, and CPU-tested admission/frame/guard/presentation policy. Remaining: reach visual parity through the authoritative shared parsed FIFO, then remove Aurora from this lane. |
