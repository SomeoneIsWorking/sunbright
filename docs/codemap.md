...
| J3D semantic masked toon materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_masked_toon_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; shipping shader: `native-render/shaders/model_masked_toon.frag.glsl`; layout adapters: `sms-recomp/overrides/semantic_j3d_adapter.cpp`, `sms-boot/runtime/native_j3d_material_adapter.cpp` | Four decoded images and four UV sets implement the reached Mario surface: 8-bit hand-mask selection between main and alternate image, a weighted toon-ramp/diffuse layer, and authored static plus directional highlights. Runtime adapters own J3D layout; the shared renderer receives only ordinary image, lighting, colour, fog, and raster values. |
