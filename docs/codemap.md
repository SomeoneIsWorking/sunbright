...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | 🟡 Explicit audit mode renders ordered pane pictures, immediate HUD pictures, and `GC2D fill_rect` values offscreen through SDL GPU; Aurora remains visible and semantic presentation is not yet owned |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | 🟡 Explicit audit mode feeds native pane/immediate pictures and `GC2D fill_rect` values through the same ordered offscreen client; Aurora remains visible and semantic presentation is not yet owned |
...
| PC-native semantic renderer | 🟡 | public schema/API: `native-render/include/`; ordered `SemanticDraw` frame/sink, picture and solid-rectangle implementations, fixed-depth J2D context stack plus active immediate canvas, renderer-neutral asset decoder, process bridge, `semantic_2d_pass.cpp`, and `sdl_semantic_frame_client.cpp`: `native-render/src/`; PC shaders and controls: `native-render/shaders/`, `native-render/tests/`; recomp picture adapter: `sms-recomp/overrides/{diag_2d,j2d_picture_adapter,semantic_j2d_context}.{h,cpp}`; recomp solid adapter/seam: `sms-recomp/overrides/solid_rectangle_adapter.{h,cpp}` and `widescreen_effects.cpp`; decomp picture/solid adapters: `sms-boot/runtime/native_{picture,solid_rectangle}_adapter.cpp`; composition/frame seams remain in each runtime | `semantic_sink` owns the exclusive producer lease and `semantic_frame_bridge` owns begin/seal lifecycle. `Semantic2dPass` walks pane pictures, immediate HUD pictures, and solid rectangles in exact submission order inside one render pass; `SdlSemanticFrameClient` owns its target/fenced submission/readback and never claims or presents a window. Runtime adapters feed copied values only and retain both original bodies. Future text/window/generic-fill ordering, scene, material, lighting, particle, and effect families belong under `native-render/`, never the GX compatibility renderer. |
