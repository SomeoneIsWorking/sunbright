...
| native renderer (SDL3 GPU) | 🟡 | `sms-recomp/runtime/render/native_render.cpp`, `native_presenter.{h,cpp}`, `native_gpu_guard.{h,cpp}`, `native_gpu_admission.{h,cpp}`, `native_gpu_pipeline.{h,cpp}`, `native_raster_state.h`, `native_efb_copy_plan.{h,cpp}`, `native_efb_copy_clear_draw.{h,cpp}`, `native_tev_uniform.{h,cpp}`, `gx_light.{h,cpp}`, `gx_texgen.{h,cpp}`, shaders, `scene.cpp`, `state_oracle.cpp` | Native owns the SDL3-GPU device, Aurora's SDL window, its swapchain, and visible presentation; Aurora presentation is disabled but its offscreen FIFO result remains the oracle. `native_gpu_pipeline` owns shader/pipeline state, `native_raster_state` owns CPU-side raster admission for GX state SDL cannot express, `native_tev_uniform` owns the shader-facing TEV layout/packing including GX RAS/TEX swap selectors and tables, `native_efb_copy_plan` owns ordered copy and post-copy policy, `native_efb_copy_clear_draw` owns the backend-independent rectangular clear draw, and `native_presenter` owns WSI. `SBR_RENDERER=native` plus the `run-render.sh`/in-process accident gate selects it. Safety is fail-closed on the first fault, bounded waits, explicit EFB-copy source validation, transactional resource replacement, and CPU-tested admission/frame/guard/presentation policy. |
...
| render A/B harness | 🔬 | `sms-recomp/runtime/render/render_compare.{h,cpp}`, `render_compare_join.{h,cpp}` + Aurora frame-sink ABI/readback job in `extern/aurora/include/aurora/aurora.h`, `extern/aurora/lib/frame_sink_schedule.hpp`, and `extern/aurora/lib/aurora.cpp` | Owns exact-frame native/Aurora scoring and controlled operation attribution. Aurora's capture-time frame identity belongs in the sink job and callback; `render_compare_join` owns the bounded exact-ID rendezvous for native baselines, variants, and delayed oracle pixels. Unknown, duplicate, or missing identities and a failed no-op control must suppress verdicts (I008). Exact pairing validates each row's delta, but the current round-robin samples different scene frames per operation, so cross-row ranking remains exploratory. |
