...
| native renderer (SDL3 GPU) | 🟡 | `sms-recomp/runtime/native_render.cpp` (+ `sms-recomp/runtime/shaders/`, `sms-recomp/runtime/gx_texture.cpp`) | geometry, per-material depth/blend, textures, TEV incl. COMPARE mode, alpha test, wrap modes. edgeIoU 29.0% / lumaCorr +0.719 vs the aurora oracle. KNOWN GAP: colour channel 1 is never evaluated and the per-stage RAS channel selector is ignored — materials read channel 1 in the stage that decides the pixel, which is why surfaces come out black. Stages are still pinned to texture unit 0 (`SBR_TEXMAP_UNITS=<mask>` opts in). See `debug_journal/2026-07-23_native_texgen_and_texmap_bisect.md` |
