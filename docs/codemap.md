...
|---|---|---|---|
| recomp | ✅ Default displayed path: parsed FIFO → Aurora → present | 🟡 Explicit `run-render.sh` diagnostic path: a project-owned GX implementation consumes the same parsed FIFO while Aurora renders offscreen as its reference | ⬜ No shipping semantic scene interface or above-GX renderer path exists yet; issue 24 owns the boundary |
| decomp | ✅ Existing `sms-boot` oracle and moddable runtime | — Not a product goal | ⬜ No native semantic adapter or shared PC renderer consumer exists yet |
...
| PC-native semantic renderer | ⬜ | target: a semantic adapter package beneath `sms-recomp/runtime/render/`; a shared root-level native-render package split into scene, materials, resources, passes, and presentation owners | Consumes game-semantic meshes/poses, materials/textures, lights/cameras, particles, UI, and named effects above GX. The decomp adapter supplies the same value schema from native objects. No FIFO/BP/XF/TEV/EFB protocol belongs across this boundary. |
| 2D / HUD overrides | 🟡 | `sms-recomp/overrides/diag_2d.cpp`, `sms-recomp/overrides/hud.cpp` | `diag_2d.cpp` is the existing J2D draw-entry census and owns the first semantic adapter at `J2DPicture::drawSelf(int, int, Mtx*)`; extend it instead of registering duplicate overrides. That adapter reads J2D values at entry and must not derive its material from post-body FIFO/TEV state. `hud.cpp` owns the current widescreen corner layout. |
