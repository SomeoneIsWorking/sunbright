...
| J3D semantic effect materials | 🟡 | shared classifier and renderer-neutral material: `native-render/{include/sunbright/native_render,src}/j3d_effect_material.*`, `native-render/{include/sunbright/native_render,src}/model.*`; shipping pipeline: `native-render/src/semantic_3d_pass.cpp`; layout adapter: `sms-recomp/overrides/semantic_j3d_adapter.cpp` | The reached lens-glow family reduces one exact primary-light/one-stage program to decoded texture times authored effect colour, with explicit additive source-alpha/destination-one blending and disabled depth testing. Unsupported effect programs remain on the retained renderer; the semantic boundary carries no TEV or register identity. |
