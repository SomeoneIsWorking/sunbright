...
bool resolve_scissor(const Canvas& canvas, const ClipRect& clip, PixelRect& scissor) noexcept {
    if (!finite(canvas.origin) || !finite(canvas.extent) || canvas.extent.x <= 0.0f ||
        canvas.extent.y <= 0.0f || canvas.targetWidth == 0 || canvas.targetHeight == 0)
        return false;
    if (!clip.enabled) {
        scissor = {0, 0, canvas.targetWidth, canvas.targetHeight};
        return true;
    }
    if (clip.width == 0 || clip.height == 0)
        return false;

    const float scaleX = static_cast<float>(canvas.targetWidth) / canvas.extent.x;
    const float scaleY = static_cast<float>(canvas.targetHeight) / canvas.extent.y;
    const auto clampX = [&](float value) {
        return std::clamp(value, 0.0f, static_cast<float>(canvas.targetWidth));
    };
    const auto clampY = [&](float value) {
        return std::clamp(value, 0.0f, static_cast<float>(canvas.targetHeight));
    };
    const float left = clampX((static_cast<float>(clip.x) - canvas.origin.x) * scaleX);
    const float top = clampY((static_cast<float>(clip.y) - canvas.origin.y) * scaleY);
    const float right = clampX(
        (static_cast<float>(clip.x) + static_cast<float>(clip.width) - canvas.origin.x) * scaleX);
    const float bottom = clampY(
        (static_cast<float>(clip.y) + static_cast<float>(clip.height) - canvas.origin.y) * scaleY);
