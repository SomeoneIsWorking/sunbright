...
constexpr std::array<std::uint8_t, 8> kTextureDiffuseStage{0xC0, 0x38, 0xF8, 0xAF,
                                                           0xC1, 0x08, 0xF2, 0xF0};
...
const char* j3d_specular_textured_result_name(J3dSpecularTexturedResult result) noexcept {
    ...
    case J3dSpecularTexturedResult::UnsupportedColorProgram:
        return "unsupported tint/specular colour program";
...
J3dSpecularTexturedResult classify_j3d_specular_textured_material(
    ...
    if (!state.lightingEnabled || state.colorChannelCount != 2 ||
        state.colorChannelControl != kDiffuseColorChannel ||
    ...
    if (state.tevStage0 != kTextureDiffuseStage || state.tevStage1 != kTintSpecularStage ||
        state.konstColorSelection0 != kKonstColor0 || state.konstColorSelection1 != kKonstColor0) {
    ...
    material.tintColor = color_from_rgba8(state.konstColorRgba8[0]);
