// boot_stubs/sdk_gap_stubs.cpp — no-op impls for GC SDK / renderer surfaces
// Aurora doesn't provide and the decomp references. Under SMS_NATIVE_PLATFORM=1
// the game's fast paths short-circuit most of these; the remaining call sites
// still need a linkable symbol. Each stub is intentionally minimal — real
// implementations replace them as the boot path exercises the feature.
...
BOOL OSSendMessage(OSMessageQueue*, OSMessage, s32) { return TRUE; }
BOOL OSReceiveMessage(OSMessageQueue*, OSMessage*, s32) { return FALSE; }
...
// ---- VI (Aurora owns present + retrace; retrace count monotonically increments so
// the game's frame pacing doesn't deadlock) ----
...
GXDrawSyncCallback GXSetDrawSyncCallback(GXDrawSyncCallback) { return nullptr; }
...
// ---- sb_* render_pc shims (retired render_pc lib; SMS_NATIVE_PLATFORM branches
// call these but Aurora owns rendering, so they're no-ops for now) ----
extern "C" {
