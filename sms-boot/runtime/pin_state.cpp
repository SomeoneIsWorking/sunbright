...
void load_once() {
    g_pin.loaded = true;
    const auto& configuredPath = sb::runtime_config().pinStatePath;
    if (configuredPath.empty())
        return;
    const char* path = configuredPath.c_str();
    FILE* f = std::fopen(path, "rb");
    if (!f) {
        sb_errorf("pin", "camera pin state '%s' is not readable", path);
        return;
    }
    char buf[4096];
    ...
    g_pin.active = ok;
    sb_infof("pin",
             "camera pin state '%s' %s: pos=(%.1f,%.1f,%.1f) tgt=(%.1f,%.1f,%.1f) "
             "up=(%.1f,%.1f,%.1f) fovy=%.1f",
