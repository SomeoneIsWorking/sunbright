...
const MMarkPoint kMMark[] = {
    {0x42440000, 0x438a8000, 0}, {0x41a00000, 0x43930000, 1}, {0x41b00000, 0x436a0000, 1},
    {0x42480000, 0x432f0000, 1}, {0x42dc0000, 0x42e00000, 1}, {0x43000000, 0x42d40000, 1},
    {0x430a0000, 0x42dc0000, 1}, {0x43060000, 0x42e20000, 1}, {0x43070000, 0x43000000, 1},
    {0x42ea0000, 0x43260000, 1}, {0x42ee0000, 0x43490000, 1}, {0x43010000, 0x43650000, 1},
    {0x430c0000, 0x435b0000, 1}, {0x43150000, 0x432c0000, 1}, {0x43320000, 0x43090000, 1},
    {0x43400000, 0x42e40000, 1}, {0x43580000, 0x42d00000, 1}, {0x43630000, 0x42da0000, 1},
    {0x43600000, 0x43020000, 1}, {0x43370000, 0x43848000, 3}, {0x43210000, 0x41900000, 0},
    {0x43210000, 0x4190147b, 8}, {0x430d0000, 0x429e0000, 3}, {0x437f0000, 0x40400000, 0},
    {0x437f0000, 0x4040a3d7, 8}, {0x435c0000, 0x428e0000, 4}, {0xbf800000, 0xbf800000, -1},
};
...
inline f32 Hx_MotionUpdate_circle() {
    return 0.0f;
} // returns G_var (render-only)
...
void run_callback() {
    ...
    default:
        ...
        if (!warned[G.type]) {
            warned[G.type] = 1;
            sb_warnf("wipe",
                     "UNIMPLEMENTED wipe type %u — port its callback (table1[%u]) before "
                     "this transition (RE from the DOL)",
                     G.type, G.type);
        }
        break;
    }
...
u32 Hx_UpdateWipe(f32 rate) {
    ...
    if (sb_log_enabled("wipe")) {
        static u32 last = 0xffffffff, lastPhase = 0xffffffff;
        if (G.state != last || G.phase != lastPhase) {
            SB_LOGC("wipe", "type=%u state=%u phase=%u timer=%u thpGate=%d seGate=%d",
                    (unsigned)G.type, (unsigned)G.state, (unsigned)G.phase, (unsigned)G.timer,
                    (int)g_flagThp, (int)g_flagSe);
            last = G.state;
            lastPhase = G.phase;
        }
    }
