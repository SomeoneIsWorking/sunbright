...
layout(location = 2) flat out vec4 vWhite;   // J2DPicture intensity black/white remap
layout(location = 3) flat out vec4 vBlack;
vec4 unpack(uint k) {
    return vec4(float((k >> 24) & 0xffu), float((k >> 16) & 0xffu),
