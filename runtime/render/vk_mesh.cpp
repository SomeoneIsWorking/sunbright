...
int sb_ngx_render(char* outbuf, int cap) {
    ...
    }
    {
        ...
        si.codeSize = sizeof kMeshFragSpv; si.pCode = kMeshFragSpv;
        if ((vr = vkCreateShaderModule(dev, &si, nullptr, &fs))) { FAIL("frag shader"); goto done; }
    }
    ...
    // ── Record ──────────────────────────────────────────────────────────────────
    {
        ...
        for (size_t b = 0; b < batches.size(); b++) {
            ...
            if (ti != last) { vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, pll, 0, 1, &tset[ti], 0, nullptr); last = ti; }
