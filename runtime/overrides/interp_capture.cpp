...
// draws set their ortho/2D matrices via direct LoadXFReg, and interpolating those mangles them) and
// did NOT fix the water jitter (so the water's swim is NOT a direct-projection-matrix lag as
// hypothesized). The fork seam sb_slot_xf_reg is left dormant (default-null, never installed) as
// zero-cost infrastructure for a future TARGETED approach (interpolate only specific matrix slots,
// excluding 2D/ortho) if one is ever justified by ground-truth RE of the water mechanism.
...
SUNBRIGHT_OVERRIDE_IF(ov_j3d_viewCalc_blend, 0x802deeb8u, getenv("SUNBRIGHT_INTERP60_REPLAY")) {
...
SUNBRIGHT_OVERRIDE_IF(ov_sdl_viewCalcSimple, 0x8023d36cu, getenv("SUNBRIGHT_INTERP60_REPLAY")) {
    if (g_interp60_in_redraw) {
...
SUNBRIGHT_OVERRIDE_IF(ov_j3dshape_draw_guard, 0x802e0390u, getenv("SUNBRIGHT_INTERP60_REPLAY")) {
    if (g_interp60_in_redraw) {
