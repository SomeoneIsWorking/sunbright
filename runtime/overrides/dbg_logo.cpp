...
SUNBRIGHT_OVERRIDE(dbg_logo_creator, 0x802a6398u) {
    ...
    if (on()) sb_trace("creat.in", self, mem_r32(self + 8), mem_r32(self + 4), mem_r32(0x8040e190));
    func_802a6398(cpu);
    if (on()) sb_trace("creat.out", self, mem_r32(self + 8), mem_r32(self + 4), mem_r32(0x8040e190));
}
...
SUNBRIGHT_OVERRIDE(dbg_logo_driver, 0x802a5f50u) {
    ...
    if (on()) sb_trace("drv.in", self, st_in, dir, ph);
    func_802a5f50(cpu);
    if (on()) sb_trace("drv.ret", self, cpu.gpr[3], mem_r32(self + 4), mem_r32(0x8040e190));
}
#endif
