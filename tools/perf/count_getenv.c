...
static void report(void) {
    ...
    if (g_total == 0) {
        fprintf(
            f, "INTERPOSER SAW NOTHING: getenv() was never intercepted. Either LD_PRELOAD did not\n"
        ...
        for (int a = 0; a < g_n; a++) {
            ...
            for (int b = a + 1; b < g_n; b++)
                if (g_cnt[b] > g_cnt[best])
                    best = b;
            long tc = g_cnt[a];
            g_cnt[a] = g_cnt[best];
            g_cnt[best] = tc;
            char tn[64];
            memcpy(tn, g_name[a], 64);
            memcpy(g_name[a], g_name[best], 64);
            memcpy(g_name[best], tn, 64);
            fprintf(f, "%10ld  %s\n", g_cnt[a], g_name[a]);
        }
    }
    if (f != stderr)
