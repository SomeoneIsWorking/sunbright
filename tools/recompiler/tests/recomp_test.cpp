...
static uint32_t enc_addi(int rt, int ra, int16_t si) {
    return (14u << 26) | (rt << 21) | (ra << 16) | (uint16_t)si;
}
static uint32_t enc_b(uint32_t from, uint32_t to, bool lk, bool aa = false) {
    int32_t d = (int32_t)(to - from);
    return (18u << 26) | ((uint32_t)d & 0x03fffffc) | (aa ? 2 : 0) | (lk ? 1 : 0);
}
static uint32_t enc_bc(uint32_t from, uint32_t to, int bo, int bi) { // conditional branch
    int32_t d = (int32_t)(to - from);
    return (16u << 26) | (bo << 21) | (bi << 16) | ((uint32_t)d & 0x0000fffc);
}
...
static std::vector<uint8_t> big_endian_bytes(const std::vector<uint32_t>& words) {
...
static Built build(uint32_t base, const std::vector<uint32_t>& words, bool cfg) {
    const std::vector<uint8_t> data = big_endian_bytes(words);
...
static int count(const std::string& s, const std::string& sub) {
    int n = 0;
    for (size_t i = s.find(sub); i != std::string::npos; i = s.find(sub, i + sub.size()))
        ++n;
    return n;
}
// A-form paired single (opcode 4): D, A, B, C and a 5-bit XO in bits 5..1.
static uint32_t enc_ps(int xo, int d, int a, int b, int c) {
    return (4u << 26) | ((uint32_t)d << 21) | ((uint32_t)a << 16) | ((uint32_t)b << 11) |
           ((uint32_t)c << 6) | ((uint32_t)(xo & 0x1f) << 1);
}
...
static uint32_t enc_x31(int xo10, int d, int a, int b) {
    return (31u << 26) | ((uint32_t)d << 21) | ((uint32_t)a << 16) | ((uint32_t)b << 11) |
           ((uint32_t)xo10 << 1);
}
...
int main() {
    ...
    // Function discovery owns whole-section direct-call target collection independently of the
    // instruction emitter. Cross-section calls stay in the target set but cannot become starts in
    // the current section.
    {
        const uint32_t external = 0x80200000u;
        const std::vector<uint32_t> words = {
            enc_b(B, B + 8, true),
            enc_b(B + 4, external, true),
            BLR,
        };
        const std::vector<uint8_t> data = big_endian_bytes(words);
        const std::vector<uint32_t> targets = find_call_targets(data.data(), B, data.size());
        CHECK(targets == std::vector<uint32_t>({B + 8, external}),
              "discovery retains sorted direct-call targets across section boundaries");
        const std::vector<uint32_t> functions = find_functions(data.data(), B, data.size());
        CHECK(functions == std::vector<uint32_t>({B, B + 8}),
              "function starts include only call targets inside the scanned section");
    }
    ...
    //    Layout:  +0 addi ; +4 b->+12 ; +8 addi(unreachable) ; +12 addi ; +16 blr
    {
        ...
        CHECK(lin.n_instrs == 5,
              "linear collects the whole body (internal branch does NOT truncate)");
    ...
    //    branch. The internal-branch truncation bug used to drop it (call never emitted →
    //    unreachable by overrides). Now linear keeps collecting past the internal `b`, so BOTH
    //    linear and CFG emit the cached direct call. (This is the HUD quad-emitter bug AND the
    //    initAllCheckData class.)
    {
        ...
        CHECK(has(lin.code, "call_ppc_direct<0x802cd2ecu>(cpu)"),
              "linear no longer truncates at the internal branch → emits the later bl (the fix)");

        Built cfg = build(B, w, true);
        CHECK(has(cfg.code, "call_ppc_direct<0x802cd2ecu>(cpu)"),
              "force-CFG emits the cached direct call to the callee (the fix)");
    }

    // 3. Branch emission forms: known bl/b targets cache their final target; blr returns.
    {
        ...
        CHECK(has(b.code, "call_ppc_direct<0x80123454u>(cpu)"), "bl emits a cached direct call");
        ...
        CHECK(has(b2.code, "call_ppc_direct<0x80200000u>(cpu); return;"),
              "b leaving the function emits a cached direct tail call");
        CHECK(!has(b2.code, "goto"), "out-of-function branch is not a goto");
    }

    // 4. Conditional backward branch (a loop) is an intra-function goto under linear collection
    // too.
    {
        std::vector<uint32_t> w = {
    ...
    //    collection USED to truncate at that forward `b`, emitting `tail_ppc(loop_cond)` and
    //    handing the whole grid-populate loop to a mid-function JIT handoff that corrupted state.
    //    The body must stay in the recompiled function: the forward `b` is a goto, the back-edge is
    //    a goto, and there is NO tail_ppc. Layout: +0 head ; +4 b->+16 (to cond) ; +8/+12 loop body
    //    ; +16 bc->+8 (back-edge) ; +20 blr
    {
        std::vector<uint32_t> w = {
        ...
        Built lin = build(B, w, /*cfg=*/false);
        CHECK(lin.n_instrs == 6,
              "initAllCheckData pattern: linear collects the whole loop function");
    ...
    // 5b. THE __va_arg / waitForRetrace REGRESSION: a `blr` (one switch-case's exit) that an
    // EARLIER
    //     forward conditional branch JUMPS OVER. Collection must NOT stop at that blr — the code
    //     after it is a later case reached via the forward `bc`. Truncating there emitted the rest
    //     as a mid-function tail_ppc → JIT handoff that, under a recomp `bl` caller,
    //     siglongjmp-unwound the caller's C frame and dropped its non-volatiles (the boot
    //     endRendering→vsnprintf→__va_arg r31 clobber: func_80337ccc `blr` @80337d14 jumped over by
    //     `bc 0x80337d18`). Layout: +0 bc->+12 (over the blr) ; +4 case1 ; +8 blr ; +12 case2 (bc
    //     target) ; +16 blr
    {
        std::vector<uint32_t> w = {
    ...
    //    paired-single IDCT (s16) → fully corrupt FMV. Pin the helper-based emission.
    {
        ...
        const uint32_t psq_st =
            (60u << 26) | (3u << 21) | (4u << 16) | (0u << 15) | (5u << 12) | 8u;
        Built b = build(0x80100000u, {psq_l, psq_st, BLR}, false);
        CHECK(has(b.code, "psq_load("),
              "psq_l emits the psq_load helper (type-correct width/stride)");
        CHECK(has(b.code, "psq_store("),
              "psq_st emits the psq_store helper (type-correct width/stride)");
    ...
    //    stay no-ops — they have no memory effect we model.)
    {
        ...
        CHECK(!has(b.code, "dcbz \xE2\x80\x94 no-op"),
              "dcbz is no longer a no-op (the THP-comb bug)");
        CHECK(has(b.code, "dcbi \xE2\x80\x94 no-op"),
              "dcbi stays a no-op (no modeled memory effect)");
    }
    ...
    //     JUT crash screen, 2026-06-10). Pin that icbi emits the icbi32 helper at (rA|0)+rB.
    {
        ...
        CHECK(has(b.code, "icbi32(cpu.gpr[4])"),
              "icbi invalidates Dolphin's icache line at (rA|0)+rB");
        CHECK(!has(b.code, "icbi \xE2\x80\x94 no-op"),
              "icbi is no longer a no-op (stale-fetch bug)");
    }

    // 9. THE BOOT JIT-HANDOFF REGRESSION: a computed `bctr` jump table whose case labels live
    // INSIDE
    //    the function must dispatch via an in-function `switch(ctr){goto}` — NOT a `tail_ppc`
    //    handoff to Dolphin's JIT (which corrupted non-volatile regs across the recomp↔JIT boundary
    //    = the boot render crash). jumptable_targets() reads the `lis/addi base; lwzx
    //    ctr,[base+idx]; mtctr; bctr` table and returns the in-function targets so they become
    //    labels + switch cases.
    {
        auto enc_addis = [](int rt, int ra, uint16_t ui) {
            return (15u << 26) | (rt << 21) | (ra << 16) | ui;
        };
        auto enc_cmpli = [](int bf, int ra, uint16_t ui) {
            return (10u << 26) | (bf << 23) | (ra << 16) | ui;
        };
        auto enc_lwzx = [](int rt, int ra, int rb) {
            return (31u << 26) | (rt << 21) | (ra << 16) | (rb << 11) | (23u << 1);
        };
        // mtspr CTR: spr field (bits11-20) = 288 so decode_spr(288)==9 (SPR_CTR)
        auto enc_mtctr = [](int rs) {
            return (31u << 26) | (rs << 21) | (288u << 11) | (467u << 1);
        };
        const uint32_t BCTR = 0x4e800420u;
        const uint32_t B = 0x80100000u;
        const uint32_t TABLE = 0x803d0000u; // a .data table address (lis 0x803d, addi 0)
        // A forward `bc` (the out-of-range bound check, here bgt→B+44) keeps LINEAR collection
        // going past the `bctr` so the bctr-only case bodies are collected — exactly the real
        // structure. +0 cmpli ; +4 bgt->B+44 ; +8 lis ; +12 addi ; +16 lwzx ; +20 mtctr ; +24 bctr
        ...
        CHECK(jt.count(B + 28) && jt.count(B + 36) && jt.count(B + 44),
              "jumptable_targets returns the table entries");
        ...
        CHECK(has(code, "case 0x8010001cu: goto lbl_8010001c;"),
              "switch dispatches CTR to the case label (no handoff)");
        ...
        CHECK(has(code, "default: tail_ppc(cpu, cpu.ctr); return;"),
              "external CTR values still tail_ppc");
    }

    // 10. JUMP-TABLE BASE BUILT IN THE PROLOGUE (TCardManager::cmdLoop 802b16b0): the table base
    //     register is materialized once at function start — `lis rX; addi rBase,rX,lo`
    //     (CROSS-register addi) — with an unconditional `b` and a `bl` between it and the `bctr`.
    //     The old 24-instr back-scan requiring `addi rD,rD` missed it → bctr emitted as tail_ppc →
    //     mid-thread-body JIT handoff killed the card worker thread → file-select menu never opened
    //     (2026-06-10). jumptable_targets must constant-propagate the base across the whole body
    //     (bl clobbers only volatile r3-r12; the nonvolatile base must survive).
    {
        auto enc_addis = [](int rt, int ra, uint16_t ui) {
            return (15u << 26) | (rt << 21) | (ra << 16) | ui;
        };
        ...
        auto enc_lwz = [](int rt, int ra, int16_t d) {
            return (32u << 26) | (rt << 21) | (ra << 16) | (uint16_t)d;
        };
        const uint32_t BCTR = 0x4e800420u;
        ...
        // +28 cmpli r0,2 ; +32 bgt -> +12 ; +36 rlwinm r0,r0,2 ; +40 lwzx r0,r31,r0 ; +44 mtctr ;
        // +48 bctr
        const uint32_t RLWINM_x4 = (21u << 26) | (0u << 21) | (0u << 16) | (2u << 11) | (0u << 6) |
                                   (29u << 1); // rlwinm r0,r0,2,0,29
        std::vector<uint32_t> w = {
        ...
        CHECK(jt.size() == 1 && jt.count(B + 12),
              "jumptable_targets resolves a prologue-built (cross-register, branch/call-separated) "
              "table base");
    }
    ...
    // live by the PSMTXConcat native-port shadow harness (2026-06-12).
    {
        ...
        CHECK(has(b.code, "(f32)std::fma"), "ps_madds emits FUSED fma rounded to single");
    ...
    }
    {
        ...
        CHECK(!has(b.code, "cpu.fpr[3].ps0"),
              "ps_sum1 must NOT read frB(ps0) — the historical bug");
    }
    {
    ...
    }
    {
        ...
        CHECK(has(sc.code, "cpu.cr[0].eq=1"),
              "stwcx. reports success in CR0[EQ] (always, single-thread)");
