...
static int recompile_mode(const DOL& dol, const std::string& out_dir) {
...
int main(int argc, char** argv) {
    ...
    // because RVZ needs Dolphin's DiscIO, and in the two-runtime architecture aurora
    ...
    if (analyze_only) {
        if (!disc) {
            std::cerr << "--analyze-only needs a disc image (it lists disc files); "
                         "a .dol has no filesystem\n";
            return 1;
        }
    ...
    return recompile_mode(dol, out_dir);
}
