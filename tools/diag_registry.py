...
PYTHON_ENV = re.compile(
    r'\b(?:os\.environ|environment)\.(?:get|pop)\(\s*"((?:SB|SBR|AURORA)_[A-Z0-9_]+)"'
)
...
LOGGER = re.compile(
    r"(?:SB_LOGC|SB_LOG_ONCE|SB_LOG_EVERY|lucent::(?:debug|info|warn|error))\s*\("
)
...
def tracked_files(dirs):
    ...
    for d in dirs:
        ...
        top = subprocess.run(
            ...
            cwd=p if os.path.isdir(p) else os.path.dirname(p),
            capture_output=True,
            text=True,
            env=env,
        ).stdout.strip()
...
def scan_code():
    ...
    for path in tracked_files(CODE_DIRS):
        if not path.endswith((".c", ".cpp", ".h", ".hpp", ".cc", ".py")):
            continue
        text = read_text(path)
        if "getenv" not in text and ".get(" not in text and ".pop(" not in text:
            continue
        lines = text.splitlines()
        for i, line in enumerate(lines, 1):
            for m in (
                *GETENV.finditer(line),
                *ENV_HELPER.finditer(line),
                *PYTHON_ENV.finditer(line),
            ):
                name = m.group(1)
...
def macro_names():
    ...
    for path in tracked_files(CODE_DIRS):
        ...
        for m in re.finditer(
            r"^\s*#\s*define\s+((?:SB|SBR|AURORA)_[A-Z0-9_]+)", text, re.M
        ):
            names.add(m.group(1))
        # Enum constants: `SB_POP_COGWHEEL = 18,` or a bare `SB_FOO,` inside an enum body.
        for m in re.finditer(
            r"^\s*((?:SB|SBR|AURORA)_[A-Z0-9_]+)\s*(?:=[^,;]*)?,\s*(?://.*)?$",
            text,
            re.M,
        ):
            names.add(m.group(1))
...
def cmd_scan(a):
    ...
    print(
        f"  {ours} in code we OWN (sms-boot/, sms-recomp/, tools/) — the real debt to convert "
        f"to the logger"
    )
    print(
        f"  {vendored} in VENDORED trees (decomp/sms, extern/aurora) — NOT a to-do: converting "
...
def cmd_phantoms(a):
    ...
    phantoms = {
        n: s
        for n, s in mentions.items()
        if n not in code
        and n not in macros
        and n not in LIBRARY_READ
        and not n.endswith("_")
    }
    # A name only ever mentioned in its own registry entry is not evidence of anything.
    phantoms = {
        n: {p for p in s if os.path.basename(p) != "diagnostics.md"}
    ...
    if not live:
        print(
            "no LIVE phantoms: every switch named in CLAUDE.md, docs/ or a run script is "
            "actually read by the code."
        )
        if historical:
            print(
                f"({historical} name(s) appear only in dated debug_journal entries — history "
    ...
    if historical:
        print(
            f"({historical} further name(s) appear only in dated journal entries — history, "
            f"not actionable.)"
        )
    return 1
...
def cmd_check(a):
    ...
    if not os.path.exists(DOC):
        ...
        print(
            f"STALE {os.path.relpath(DOC, ROOT)} — switches changed; run `diag_registry.py write`"
        )
        rc = 1
    if cmd_phantoms(a) != 0:
        rc = 1
...
def main():
    ...
    s.add_argument("--gated-prints", action="store_true")
    s.set_defaults(fn=cmd_scan)
    s = sub.add_parser("phantoms")
    s.set_defaults(fn=cmd_phantoms)
