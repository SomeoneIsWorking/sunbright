...
def texture_manifest_lines(output: str) -> list[str]:
    return [
        line for line in output.splitlines() if line.startswith("[texresolve] static ")
    ]
...
def validated_limits(environment: dict[str, str]) -> tuple[float, float, int]:
    ...
    if not math.isfinite(present_hz) or not 0 < present_hz <= MAX_PRESENT_HZ:
        raise ValueError(
            f"SB_MAX_PRESENT_HZ must be finite and in (0,{MAX_PRESENT_HZ:g}]"
        )
...
def run(invocation: Invocation) -> int:
    ...
    if invocation.renderer_was_forced:
        print(
            "[run-safe] native renderer input was set. This wrapper runs the Aurora lane; use",
            file=sys.stderr,
        )
        print(
            "           ./run-render.sh for the gated native preview. Forcing "
            "SBR_RENDERER=aurora.",
            file=sys.stderr,
        )
    ...
    print(
        f"[run-safe] present ceiling {os.environ['SB_MAX_PRESENT_HZ']} Hz, {display_mode}, "
    ...
    try:
        ...
        print(
            f"[run-safe] {exc}. Refusing to run anything rather than silently falling back "
    ...
    if radv_hang_enabled(os.environ):
        print(
            "[run-safe] RADV hang diagnostics ENABLED: driver synchronization may mask "
    ...
    if "texresolve" not in selected and "all" not in selected:
        os.environ["LUCENT_DEBUG"] = ",".join(
            filter(None, (debug_channels, "texresolve"))
        )
    ...
    try:
        ...
        if lines:
            ...
            print(
                f"[run-safe] texture manifest beside the dump: {manifest} "
                f"({len(lines)} texture(s), {single_level} single-level)"
            )
        else:
            print(
                "[run-safe] NO texture manifest was captured, so this dump is NOT comparable "
                "against",
                file=sys.stderr,
            )
            print(
                "           another one for issue #5. That is a broken capture, not a clean "
                "run.",
                file=sys.stderr,
            )
        print(f"[run-safe] guarded game exit={result.returncode}")
...
def selftest() -> int:
    ...
    assert not radv_hang_enabled(clean.environment)

    radv = parse_invocation(["SBR_RADV_HANG_DIAG=1"], {"RADV_DEBUG": "zerovram"})
    assert radv.environment["RADV_DEBUG"] == "zerovram,hang"
    assert radv_hang_enabled(radv.environment)
    for invalid_radv in (
        ["SBR_RADV_HANG_DIAG=yes"],
        ["RADV_DEBUG=hang"],
    ):
        try:
            parse_invocation(invalid_radv, {})
        except ValueError:
            pass
        else:
            raise AssertionError(
                f"unsafe RADV hang policy was accepted: {invalid_radv}"
            )

    forced = parse_invocation(
        [
            "SBR_RENDERER=native",
            "SB_MAX_PRESENT_HZ=30",
            "rom.rvz",
            "--",
            "X=runner-arg",
        ],
    ...
    hostile = parse_invocation(["SB_HEADLESS=0", "SBR_MUTE=0"], {})
    ...
    for name, value in (
        ("SB_MAX_PRESENT_HZ", "0"),
        ("SB_MAX_PRESENT_HZ", "61"),
        ("SBR_QUIT_AFTER", "0"),
        ("SBR_QUIT_AFTER", "10001"),
    ):
    ...
    for runner_name in APPROVED_RUNNERS:
        ...
        radv_capture_at = source.index("_SUNBRIGHT_SAFE_RADV_DEBUG_CAPTURE=")
        radv_restore_at = source.index(
            'export RADV_DEBUG="$_SUNBRIGHT_SAFE_RADV_DEBUG_CAPTURE"'
        )
        assert capture_at < dotenv_at < restore_at
        assert radv_capture_at < dotenv_at < radv_restore_at
