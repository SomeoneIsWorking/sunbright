#!/usr/bin/env bash
# title_oracle.sh — capture the Dolphin-GX ground-truth render of the stage-15
# title screen (option.arc) screen, the oracle for sms-boot's native title screen fidelity.
#
...
# blowing past the settled title screen into the opening movie). Then the intro camera settles
# on the 3-block picker and waits there forever (no further input), so we dump a settled frame.
#
# Output: scratch/oracle/title_gx_oracle.png (full-res GX frame).
# Compare against sms-boot: build-native/sms-boot with SB_STAGE=15 (see title_smsboot.png).
#
# Usage: tools/render/title_oracle.sh
set -uo pipefail
HERE="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$HERE"
DUMP="$HOME/.local/share/dolphin-emu/Dump/Frames"
OUT="scratch/oracle/title_gx_oracle.png"
...
SUNBRIGHT_FASTBOOT=1 SUNBRIGHT_STAGE=15 SUNBRIGHT_SCENARIO=0 SUNBRIGHT_NGX_PRESENT=0 \
  ...
  timeout -s KILL 240 ./run.sh > scratch/frames/title_oracle.log 2>&1 &
