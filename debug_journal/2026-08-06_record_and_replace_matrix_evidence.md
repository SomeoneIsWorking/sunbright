# 2026-08-06 — Record-and-replace matrix evidence

## What changed

The sub-frame gains a second, structurally different implementation, selected by
`SBR_INTERP60_REPLACE=1` in the then-current execution path. It used the already-established
record-final-matrices presentation pattern instead of mutating simulation state.
...
That model does not have the failure mode, and not because it enumerated better: the sim
...
The reference implementation edits ~20 draw call sites because it owns decomp source. The former
guest path instead used a
