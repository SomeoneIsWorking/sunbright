# Escape settings ownership

The earlier RmlUi work stopped at a prelaunch page. The replacement used a
`Document → Window → panes/controls` subsystem, and its in-game input
route owns a modal window lifetime. Sunbright adopted the same ownership split in its UI subsystem.
